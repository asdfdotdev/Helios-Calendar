	...
	if(!isset($_GET['clear'])){
		...
		if(isset($_POST['cookieme'])){
			if($_SESSION['hc_favCat'])
				setcookie($hc_cfg[000].'_fn', base64_encode($_SESSION['hc_favCat']), time()+604800, '/', false, 0);
			
			if($_SESSION['hc_favCity'])
				setcookie($hc_cfg[000].'_fc', base64_encode(implode(',',$_SESSION['hc_favCity'])), time()+604800, '/', false, 0);
		} else {
			setcookie($hc_cfg[000].'_fn', '', time()-86400, '/', false, 0);
			setcookie($hc_cfg[000].'_fc', '', time()-86400, '/', false, 0);
		}
