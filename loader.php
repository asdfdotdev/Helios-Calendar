 ...
 */
	...
	if(!checkdate($month, $day, $year) || (strtotime(SYSDATE) > strtotime($date) && $hc_cfg[11] == 0) || $hc_cfg['First'] > date("U",mktime(0,0,0,$month+1,0,$year)) || $hc_cfg['Last'] < date("U",mktime(0,0,0,$month,1,$year))){
		$date = SYSDATE;
		...
		switch($com){
			...
			case 'send':
				if($lID > 0){
					...
					$result = doQuery("SELECT Title, StartDate, StartTime, TBD FROM " . HC_TblPrefix . "events WHERE PkID = '" . $eID . "' AND StartDate >= '" . SYSDATE . "'");
					if(hasRows($result)){
				...
				break;
			case 'rsvp':
				$result = doQuery("SELECT Title, StartDate, StartTime, TBD, ContactName, ContactEmail FROM " . HC_TblPrefix . "events WHERE PkID = '" . $eID . "' AND StartDate >= '" . SYSDATE . "'");
