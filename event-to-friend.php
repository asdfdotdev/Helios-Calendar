 ...
 */
	...
	if(hasRows($result) && $myName != '' && $myEmail != '' && $friendName != '' && $friendEmail != ''){
		...
		$message .= '<p><b>' . hc_mysql_result($result,0,0) . '</b><br />';
		
		if($tID == 0){
			$where = '/index.php?com=send&eID=';
			$subject = CalName . " " . $hc_lang_sendtofriend['SubjectE']  . " " . $myName;
			$message .=  stampToDate(hc_mysql_result($result,0,1), $hc_cfg[14]) . ' - ';
			if(hc_mysql_result($result,0,3) == 0)
				$message .= stampToDate("1980-01-01 " . hc_mysql_result($result,0,2), $hc_cfg[23]);
			...
			$message .= buildAddress(hc_mysql_result($result,0,1),hc_mysql_result($result,0,2),hc_mysql_result($result,0,3),hc_mysql_result($result,0,4),hc_mysql_result($result,0,5),hc_mysql_result($result,0,6),$hc_lang_config['AddressType']);
