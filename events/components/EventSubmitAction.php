	...
	if($stop == 0){
		...
		if($newPkID > 0){
			...
			if(hasRows($resultE)){
				...
				if($adminMessage != ''){
					$message .= '<br><br><b>' . $hc_lang_event['NoticeEmail4'] . '</b> ' . cIn(strip_tags(cleanBreaks($_POST['adminmessage'])));
				}//end if
		...
		exit('There was a problem with your event submission. If you have disabled JavaScript please enable it and try your submission again.');
	}//end if
?>