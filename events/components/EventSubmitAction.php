 ...
 */
	...
	$contactURL = htmlspecialchars(strip_tags($_POST['contactURL']));
	...
	$newPkID = 0;
	$contactURL = (preg_match('/^https?:\/\//',$contactURL)) ? $contactURL : "http://" . $contactURL;
	$allowRegistration = htmlspecialchars($_POST['eventRegistration']);
	$adminMessage = (isset($_POST['adminmessage'])) ? cIn(htmlspecialchars(cleanQuotes($_POST['adminmessage']))) : '';
	...
	if(isset($dateS[2])){
		$eventDate = dateToMySQL(htmlspecialchars(strip_tags($_POST['eventDate'])), $hc_cfg24);
		$stop += ($eventDate != '') ? 0 : 1;
	...
	if($stop == 0){
		...
		if($newPkID > 0){
			...
			if(hasRows($resultE)){
				...
				$subject = $hc_lang_event['NoticeSubject'] . ' - ' . CalName;
				$message = '<p>' . $hc_lang_event['NoticeEmail1'] . '</p>';
				$message .= '<p><b>' . $hc_lang_event['NoticeEmail2'] . '</b> ' . $subName . ' - ' . $subEmail . '<br />';
				$message .= '<b>' . $hc_lang_event['NoticeEmail3'] . '</b> ' . $_SERVER['REMOTE_ADDR'] . '</p>';
				$message .= ($adminMessage != '') ? '<p><b>' . $hc_lang_event['NoticeEmail4'] . '</b> ' . str_replace('<br />', ' ', strip_tags(cleanBreaks($_POST['adminmessage']),'<br>')) . '</p>' : '';

				$message .= '<p><b>' . $hc_lang_event['Location'] . '</b> ';
				if($locID == 0){
					$message .= $locName . ', ';
				...
				$message .= '<p><b>' . $hc_lang_event['EventTitle'] . '</b> ' . $eventTitle . '</p><p>' . strip_tags($eventDesc) . '</p>';
