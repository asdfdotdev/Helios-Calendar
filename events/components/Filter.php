...
*/
	...
	echo '<br />';
	echo (isset($_SESSION[$hc_cfg00 . 'hc_favCat']) || isset($_SESSION[$hc_cfg00 . 'hc_favCity'])) ? $hc_lang_filter['NoticeSet'] : $hc_lang_filter['Notice'];
	...
	<?php	$cities = getCities();
		...
		<?php 	$cnt = 1;
				...
				$actCities = (isset($_SESSION[$hc_cfg00 . 'hc_favCity'])) ? urldecode($_SESSION[$hc_cfg00 . 'hc_favCity']) : '';
				...
				foreach($cities as $val){
					...
					if($val != ''){	
						...
						echo '<input onclick="updateLink();" name="cityName[]" id="cityName_' . str_replace(" ","_",$val) . '" type="checkbox" value="' . urlencode($val) . '" class="noBorderIE"';
	...
	<?php	$query = NULL;
			if(isset($_SESSION[$hc_cfg00 . 'hc_favCat'])){
				$query = "SELECT c.PkID, c.CategoryName, c.ParentID, c.CategoryName as Sort, c2.PkID as Selected
							FROM " . HC_TblPrefix . "categories c 
								LEFT JOIN " . HC_TblPrefix . "categories c2 ON (c.PkID = c2.PkID AND c.PkID IN (" . $_SESSION[$hc_cfg00 . 'hc_favCat'] . "))
							...
							FROM " . HC_TblPrefix . "categories c 
								...
								LEFT JOIN " . HC_TblPrefix . "categories c3 ON (c.PkID = c3.PkID AND c.PkID IN (" . $_SESSION[$hc_cfg00 . 'hc_favCat'] . "))
								LEFT JOIN " . HC_TblPrefix . "eventcategories ec ON (c.PkID = ec.CategoryID)
							WHERE c.ParentID > 0 AND c.IsActive = 1
							GROUP BY c.PkID 
