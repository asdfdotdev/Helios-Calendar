...
*/	
	...
	$result = doQuery("	SELECT PkID, Title, StartDate, StartTime, IsBillboard, SeriesID, TBD
						...
						WHERE IsActive = 1 AND IsApproved = 1 AND StartDate >= '" . date("Y-m-d",mktime($hourOffset,date("i"),date("s"),date("m"),date("d"),date("Y"))) . "' ORDER BY Views DESC LIMIT " . cIn($hc_cfg10));
	if(hasRows($result)){
		$curDate = '';
		$cnt = 0;
