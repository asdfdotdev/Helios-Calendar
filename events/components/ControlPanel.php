...
?>		<tr>
		...
		<?php 	$jumpMonth = date("n", mktime(0,0,0,$curMonth-12,1,$curYear));
				...
				for($i = 0; $i <= 24; $i++){	?>
				<option <?php if($i == 12){echo "selected=\"selected\"";}?> value="<?php echo CalRoot;?>/index.php?year=<?php echo date("Y", mktime(0,0,0,$jumpMonth + $i,1,$jumpYear));?>&amp;month=<?php echo date("n", mktime(0,0,0,$jumpMonth + $i,1,$jumpYear)) . $locSaver;?>"><?php echo date("M Y", mktime(0,0,0,$jumpMonth + $i,1,$jumpYear));?></option>
		<?php 	}//end for?>
			</select>
			</td>
