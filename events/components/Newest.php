...
*/
	...
	$result = doQuery("	SELECT PkID, Title, StartDate, StartTime, IsBillboard, SeriesID, TBD
						...
						WHERE IsActive = 1 AND IsApproved = 1 AND StartDate >= '" . date("Y-m-d",mktime($hourOffset,date("i"),date("s"),date("m"),date("d"),date("Y"))) . "'
                              ORDER BY PublishDate DESC, StartDate LIMIT " . cIn($hc_cfg66));
	if(hasRows($result)){
		$curDate = '';
		$cnt = 0;
		...
		while($row = mysql_fetch_row($result)){
			...
			if($hc_cfg15 == 1){
				...
				echo '<li class="newest"><a href="' . CalRoot . '/index.php?com=detail&amp;eID=' . $row[0] . '" rel="nofollow" class="newest">' . cOut($row[1]) . '</a> - ' . $startTime . '</li>';
			} else {
				echo '<li class="newest"><a href="' . CalRoot . '/index.php?com=detail&amp;eID=' . $row[0] . '" rel="nofollow" class="newest">' . cOut($row[1]) . '</a></li>';
			}//end if
			$cnt = $cnt + 1;
		}//end while
		echo '</ul>';
	} else {
		echo '<br />' . $hc_lang_event['NoNewest'];
	}//end if?>