...
*/
	...
	$cacheFile = date("Ymd", mktime($hourOffset,0,0,date("m"),date("d"),date("Y"))) . '_2.php';
	if(!file_exists(realpath($incPrefix . 'cache/list' . $cacheFile))){
		if(count(glob($incPrefix . 'cache/list*_2.*')) > 0){
			foreach(glob($incPrefix . 'cache/list*_2.*') as $filename){
				unlink($filename);
			}//end foreach
		}//end if
		...
		$result = doQuery("	SELECT PkID, Title, StartDate, StartTime, IsBillboard, SeriesID, TBD, EndTime
						...
						ORDER BY PublishDate DESC, StartDate LIMIT " . cIn($hc_cfg66 * 3));
		if(hasRows($result)){
			$curDate = '';
			$cnt = $showHeader = 0;
			$hideSeries = array();
			...
			while($row = mysql_fetch_row($result)){
				if($cnt >= $hc_cfg66){break;}

				if($row[5] == '' || !in_array($row[5], $hideSeries)){
					if($curDate != $row[2]){
						$curDate = cOut($row[2]);
						echo '<li class="newestDate"><br />' . stampToDate($row[2], $hc_cfg14) . '</li>';
					}//end if

					echo '<li class="newest"><div class="hc_align"><a href="' . CalRoot . '/index.php?com=detail&amp;eID=' . $row[0] . '" rel="nofollow" class="newest">' . cOut($row[1]) . '</a></div>';
					if($hc_cfg15 == 1){
						$startTime  = $endTime = '';
						if($row[6] == 0){
			...
			echo '<p>' . $hc_lang_event['NoNewest'] . '</p>';
		}//end if
