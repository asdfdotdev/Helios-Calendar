...
*/
	...
	include('../' . $hc_langPath . $_SESSION[$hc_cfg00 . 'LangSet'] . '/public/comment.php');
     ...
     $tweet = (isset($_GET['tweet'])) ? substr(strip_tags(str_replace("\r","",str_replace("\n"," ",urldecode($_GET['tweet'])))),0,109) : '';
     ...
     if($eID > 0 && $tweet != ''){
		...
		require_once('../includes/api/bitly/ShortenURL.php');
          ...
          header('Location: http://twitter.com/home?status=' . urlencode($tweet . $hash . ' ' . $shortLink));
     } else {
