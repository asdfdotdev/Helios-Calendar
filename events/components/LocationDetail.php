 ...
 */
	...
	if(hasRows($result)){
		...
		if($locEmail != ''){	
			...
			echo '<br /><br /><div class="hc_align">' . $hc_lang_locations['Email'] . '</div>';
			cleanEmailLink($locEmail);
		}//end if
			
		echo ($locPhone != '') ? '<br /><div class="hc_align">' . $hc_lang_locations['Phone'] . '</div>&nbsp;' . $locPhone . '&nbsp;' : '';
		echo ($locURL != '' && $locURL != 'http://') ? '<br /><div class="hc_align">' . $hc_lang_locations['Website'] . '</div>&nbsp;<a href="' . CalRoot . '/link/index.php?tID=4&amp;oID=' . $locID . '" target="_blank" class="eventMain">' . $hc_lang_locations['Visit'] . '</a>&nbsp;' : '';
