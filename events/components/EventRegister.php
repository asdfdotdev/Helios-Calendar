...
*/
	...
	<div id="nav-top"><a href="<?php echo CalRoot;?>/index.php?com=detail&eID=<?php echo $eID;?>" class="eventMain">&laquo;&laquo;&nbsp;<?php echo $hc_lang_register['ReturnLink'];?></a></div>
	<br />
<?php
	$result = doQuery("SELECT Title, StartDate, ContactName, ContactEmail FROM " . HC_TblPrefix . "events WHERE PkID = " . cIn($eID) . " AND StartDate >= '" . date("Y-m-d") . "'");
	if(hasRows($result)){	
		if (isset($_GET['msg'])){
			switch ($_GET['msg']){
				case "1" :
...
<?php	}//end if	?>		
		<fieldset>
			...
			<div class="frmReq">
				...
				<?php echo stampToDate(mysql_result($result,0,1), $hc_dateFormat);?>
			</div>
			<div class="frmReq">
