...
<?	$result = doQuery("SELECT Title, StartDate, ContactName, ContactEmail FROM " . HC_TblPrefix . "events WHERE PkID = " . cIn($eID) . " AND StartDate >= NOW()");
	...
	<?	if(in_array(3, $hc_captchas)){	?>
		...
		<?php	buildCaptcha();	?><br />
			...
			<div class="frmReq">
				<label>&nbsp;</label>
				<input name="proof" id="proof" type="text" maxlength="8" size="8" value="" onblur="alert('Please verify you have entered the authentication text correctly or you will have to re-enter your contact information to register.');" />
				<-- Enter Image Text Here
			</div>
	<?	}//end if	?>
			<div class="frmReq">
				<label for="hc_f1">Name:</label>
				<input name="hc_f1" id="hc_f1" type="text" size="25" maxlength="50" value="<?if(isset($_GET['name'])){echo $_GET['name'];}//end if?>" />
			</div>
			<div class="frmReq">
				<label for="hc_f2">Email:</label>
				<input name="hc_f2" id="hc_f2" type="text" size="35" maxlength="75" value="<?if(isset($_GET['email'])){echo $_GET['email'];}//end if?>" />
			</div>
			<div class="frmOpt">
				<label for="hc_f3">Phone:</label>
				<input name="hc_f3" id="hc_f3" type="text" size="20" maxlength="25" value="<?if(isset($_GET['phone'])){echo $_GET['phone'];}//end if?>" />
			</div>
			<div class="frmOpt">
				<label for="hc_f4">Address:</label>
				<input name="hc_f4" id="hc_f4" type="text" size="20" maxlength="75" value="<?if(isset($_GET['address'])){echo $_GET['address'];}//end if?>" />
			</div>
			<div class="frmOpt">
				<label>&nbsp;</label>
				<input name="hc_f5" id="hc_f5" type="text" size="20" maxlength="75" value="<?if(isset($_GET['address2'])){echo $_GET['address2'];}//end if?>" />
			</div>
			<div class="frmOpt">
				<label for="hc_f6">City:</label>
				<input name="hc_f6" id="hc_f6" type="text" size="20" maxlength="50" value="<?if(isset($_GET['city'])){echo $_GET['city'];}//end if?>" />
			</div>
			<div class="frmOpt">
			...
			<div class="frmOpt">
				...
				<input name="hc_f8" id="hc_f8" type="text" size="11" maxlength="11" value="<?if(isset($_GET['zip'])){echo $_GET['zip'];}//end if?>" />
			</div>
		</fieldset>
		<br />
		<input name="submit" id="submit" type="submit" value="Register Now" class="button" />&nbsp;&nbsp;&nbsp;
		<input name="cancel" id="cancel" type="button" value=" Cancel " onclick="document.location.href='<?echo CalRoot;?>/index.php?com=detail&eID=<?echo $eID;?>'; return false;" class="button" />
		</form>
	<?	} else {	?>
