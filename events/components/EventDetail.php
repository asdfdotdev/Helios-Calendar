	...
	if(hasRows($resultED)){
		...
		//<!--
			...
			function tweetMe(msg,chk){
				...
				window.open('<?php echo CalRoot;?>/components/TweetMe.php?eID=' + <?php echo cIn($eID);?> + '&going=' + going + '&tweet=' + msg,'hc_shareEvent','location=1,status=1,scrollbars=1,width=800,height=600,left='+(screen.availWidth/2-400)+',top='+(screen.availHeight/2-300));
			}//end tweetMe()
		//-->
		</script>
<?php
		...
		$tweetThis = urlencode($title . ' @ ' . $locName . ' - ' . $tweetTime . ' ' . $hc_lang_event['On'] . ' ' . stampToDate($eventDate,$hc_cfg24));
          echo '<div class="socialT"><a href="javascript:;" onclick="tweetMe(\'' . $tweetThis . '\',1);return false;" class="eventDetailLink"><img src="' . CalRoot . '/images/share/twitter.png" alt="tweet this event" title="Twitter" style="vertical-align:middle;" /></a></div>';
		...
		echo '<div style="clear:both;"><a href="' . CalRoot . '/index.php?com=send&amp;eID=' . $eID . '" class="eventDetailLink"><img class="eventDetailLink" src="' . CalRoot . '/images/icons/iconEmail.png" width="16" height="16" alt="' . $hc_lang_event['ALTEmail'] . '" style="vertical-align:middle;" /></a>&nbsp;<a href="' . CalRoot . '/index.php?com=send&amp;eID=' . $eID . '" class="eventDetailLink">' . $hc_lang_event['EmailToFriend'] . '</a></div>';
		...
		$starttimepart = explode(":", $eventStartTime);
		$startdatepart = explode("-", $eventDate);
		$endtimepart = explode(":", $eventEndTime);
		if($eventStartTime != ''){
