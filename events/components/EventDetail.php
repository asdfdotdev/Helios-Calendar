 ...
 */
	if(!isset($hc_cfg00)){header("Location: ../index.php");exit();}
	...
	include_once('includes/agents.php');
	
	if(hasRows($resultED)){
		if(!preg_match($hc_bots,$_SERVER['HTTP_USER_AGENT']) && !in_array($eID,$_SESSION['hc_trail'])){
		...
		$shortURL = cOut(mysql_result($resultED,0,38));
		...
		//<!--
			...
			function tweetMe(msg){
				window.open('<?php echo CalRoot;?>/components/TweetMe.php?eID=' + <?php echo $eID;?> + '&going=' + going + '&tweet=' + escape(msg),'hc_shareEvent','location=1,status=1,scrollbars=1,width=800,height=600,left='+(screen.availWidth/2-400)+',top='+(screen.availHeight/2-300));
			}//end tweetMe()
		//-->
		</script>
<?php
          ...
          echo '</div>';
		echo '<div id="evernote">';
		echo ($eventDesc != '') ? '<div id="eventDetailDesc" class="description">' . $eventDesc . '</div>' : '<p>&nbsp;</p>';
		...
		$resultC = doQuery("SELECT c.PkID, c.CategoryName
							...
							WHERE c.IsActive = 1 AND ec.EventID = '" . $eID . "'
		...
		if(($contactURL != '' && $contactURL != 'http://') || ($contactName != '') || ($contactEmail != '') || ($contactPhone != '')){
			...
			if($contactName != '' || $contactEmail){
				...
				if($contactEmail != ''){
					...
					cleanEmailLink($contactEmail,cOut(strip_tags($eventTitle)));
					echo '&nbsp;';
				}//end if
			}//end if
		...
		$tweetThis = cIn($eventTitle) . ' @ ' . cIn($locName) . ' - ' . $tweetTime . ' ' . $hc_lang_event['On'] . ' ' . stampToDate($eventDate,$hc_cfg24) . ' ' . $hc_cfg59;
		
		echo '<div class="eventDetailToolbox">';
		echo '<div class="socialT"><a href="http://twitter.com/share" class="twitter-share-button" data-url="' . CalRoot . '/index.php?com=detail&eID=' . $eID . '" data-text="' . cOut($tweetThis) . '" data-count="horizontal" data-via="' . $hc_cfg63 . '">Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script></div>';
		...
		echo '<a href="' . CalRoot . '/index.php?com=send&amp;eID=' . $eID . '" class="eDetailShareE">' . $hc_lang_event['EmailToFriend'] . '</a>';
		echo ($shortURL != '') ? '<br /><a href="' . $shortURL . '.qrcode" class="eDetailShareQR" target="_blank" rel="nofollow">' . $hc_lang_event['QRCode'] . '</a>' : '';
		echo '</div>';

		echo '<div class="hc_align">';
		echo '<a target="_blank" class="eventShare" href="http://del.icio.us/post?url=' . $link . '&amp;title=' . $title . '"><img src="' . CalRoot . '/images/share/delicious.png" alt="Delicious" title="Delicious" style="vertical-align:middle;" /></a>';
		...
		echo '<a href="javascript:" class="eventShare" onclick="Evernote.doClip({';
		echo "title: '" . str_replace(":","",$eventTitle) . " - " . stampToDate($eventDate,$hc_cfg24) . "',";
		echo "url: '" . CalRoot . "/index.php?eID=" . $eID . "',code: 'Refr6223',contentId: 'evernote',footer: '<img src=&quot;/about/media/img/logo.png&quot; />',suggestNotebook: '" . $hc_lang_event['Events'] . "',suggestTags: '" . $hc_lang_event['Events'] . "," . CalName . "',providerName: '" . CalName . "',";
		echo ($locLat != '' && $locLon != '') ? "latitude: " . $locLat . ",longitude: " . $locLon . "," : '';
		echo "styling: 'full' });return false\">";
		echo '<img src="' . CalRoot . '/images/share/evernote.png" alt="Evernote" title="Evernote" style="vertical-align:middle;" /></a>';
		...
		echo '<a target="_blank" class="eventShare" href="http://technorati.com/faves?add=' . $link . '"><img src="' . CalRoot . '/images/share/technorati.png" alt="Technorati" title="Technorati" style="vertical-align:middle;" /></a>';
          echo '<a target="_blank" class="eventShare" href="http://www.tumblr.com/share?v=3&amp;u=' . $link . '&amp;t=' . $title . '&amp;s="><img src="' . CalRoot . '/images/share/tumblr.png" alt="tumblr" title="tumblr" style="vertical-align:middle;" /></a>';
		echo '</div>';
		echo '<div class="hc_align" style="line-height:25px;">&nbsp;<a class="eventMain" href="javascript:;" onclick="togThis(\'eventDetailShareMore\', \'eventShareLink\');" id="eventShareLink">' . $hc_lang_event['More'] . '</a>&nbsp;</div>';
		...
		echo '<a target="_blank" class="eventShare" href="http://buzz.yahoo.com/submit/?submitHeadline=' . $title . '&amp;submitUrl=' . $link . '"><img src="' . CalRoot . '/images/share/yahoobuzz.png" alt="yahoo buzz" title="yahoo buzz" /></a>';
		...
		echo '</div>';
          ...
          echo '<div class="toolboxHeader"><b>' . $hc_lang_event['SaveToCalendar'] . '</b></div>';
		echo '<a href="' . CalRoot . '/link/SaveEvent.php?eID=' . $eID . '&amp;cID=3" class="eDetailSaveI">iCalendar</a>';
		echo '<a href="' . CalRoot . '/link/SaveEvent.php?eID=' . $eID . '&amp;cID=4" class="eDetailSaveV">vCalendar</a><br />';
		...
		echo '</div>';
		echo '<br /><iframe src="http://www.facebook.com/plugins/like.php?href=' . $link . '&amp;layout=standard&amp;show_faces=false&amp;width=350&amp;action=like&amp;font=verdana&amp;colorscheme=light&amp;height=35" scrolling="no" frameborder="0" style="border:none;overflow:hidden;width:100%;height:40px;" allowTransparency="true"></iframe>';
		...
		if($hc_cfg56 == 1){
			...
			if(!isset($_SESSION['hc_OpenID'])){
				...
				echo '<input name="eID" id="eID" type="hidden" value="' . $eID . '" />';
				...
				echo '<a href="' . $_SESSION['hc_OpenID'] . '" class="commentIdentity" target="_blank">' . $_SESSION['hc_OpenIDShortName'] . '</a></div>';
