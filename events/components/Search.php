...
*/
	...
	endDate = document.eventSearch.endDate.value;
		if(!isDate(document.eventSearch.startDate.value, '<?echo $hc_popDateValid;?>')){
			dirty = 1;
			warn = warn + '\n*Start Date is Invalid Date or Format. Required Format: <?echo strtolower($hc_popDateFormat);?>';
		}//end if 

		if(!isDate(document.eventSearch.endDate.value, '<?echo $hc_popDateValid;?>')){
			dirty = 1;
			warn = warn + '\n*End Date is Invalid Date or Format. Required Format: <?echo strtolower($hc_popDateFormat);?>';
		}//end if 
	...
	<br />
	<fieldset>
		<legend>Search Keywords (Optional)</legend>
		<div class="frmOpt">
			<label for="keyword">Keyword:</label>
			<input name="keyword" id="keyword" type="text" size="25" maxlenght="50" value="" />
		</div>
	</fieldset>
	<br />
	<fieldset>
	...
	<fieldset>
		...
		<div class="frmOpt">
			<label>&nbsp;</label>
			<input type="submit" name="submit" id="submit" value=" Begin Search " />
		</div>
	</fieldset>
	</form>