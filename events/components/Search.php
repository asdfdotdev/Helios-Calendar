...
*/
	...
	<br />
	<fieldset>
		<legend>Search Locations (<a href="javascript:;" onclick="togLocation(1);" class="eventMain">Location</a> OR <a href="javascript:;" onclick="togLocation(2);" class="eventMain">City</a>)</legend>
		<div class="frmOpt">
		...
		<div class="frmOpt">
			...
			<?	$result = doQuery("SELECT e.LocationCity, l.City, l.IsActive
									...
									WHERE 
										...
										e.IsApproved = 1 AND 
										e.StartDate >= NOW()) AND
										(l.IsActive = 1 OR l.IsActive is NULL)");
				$cities = array();
				while($row = mysql_fetch_row($result)){
					if($row[1] != ''){
						$cities[] = $row[1];
