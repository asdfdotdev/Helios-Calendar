...
*/
	...
	if($locName != ''){
		if(!isset($_GET['a'])){
			...
			$result = doQuery("SELECT PkID, Name, Address, Address2, City, State, Zip, Country
								...
								WHERE NAME LIKE('%" . cIn($locName) . "%')" . $pubOnly . " AND IsActive = 1
								ORDER BY Name 
								LIMIT " . $resLimit . " OFFSET " . ($resOffset * $resLimit));
			
			$resultP = doQuery("SELECT COUNT(PkID) FROM " . HC_TblPrefix . "locations WHERE NAME LIKE('%" . cIn($locName) . "%')" . $pubOnly . " AND IsActive = 1");
		} else {
			$result = doQuery("SELECT DISTINCT(l.PkID), l.Name, l.Address, l.Address2, l.City, l.State, l.Zip, l.Country
								...
								WHERE l.NAME LIKE('%" . cIn($locName) . "%') AND l.IsActive = 1
	...
	if(isset($result) && hasRows($result)){
		...
		while($row = mysql_fetch_row($result)){
			...
			echo '<label class="locSearch' . ($x % 2). '" for="locValue_' . $row[0] . '"><input name="locValue" id="locValue_' . $row[0] . '" type="radio" onclick="setLocation(' . $row[0] . ',\'' . htmlentities(str_replace('\'','\\\'',$row[1]),ENT_QUOTES) . '\', 1);" class="noBorderIE" />';
			echo '<span class="hc_align"><b>' . htmlentities($row[1],ENT_QUOTES) . '</b></span>';
			echo ($row[2] != '' && $row[4] != '' && $row[5] != '') ? '<span class="hc_align">&nbsp;<a href="' . CalRoot . '/link/index.php?tID=2&oID=24&lID=' . $row[0] . '" class="eventMain" target="_blank"><img src="' . CalAdminRoot . '/images/icons/iconMap.png" width="16" height="16" alt="' . $hc_lang_search['Map'] . '"  style="vertical-align:top;" /></a>&nbsp;</span>' : '';
			echo '<br />';
			...
			$locAddress = str_replace('<br />','<span class="hc_align">,&nbsp;</span>',$locAddress);
		...
		echo '<label style="clear:both;">&nbsp;</label>';
		echo '<span class="noLocSearch">' . $hc_lang_search['NoLocation'] . '</span>';
	}//end if