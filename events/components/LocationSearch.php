...
*/
	...
	if(isset($result) && hasRows($result)){
		...
		while($row = mysql_fetch_row($result)){
			...
			echo '<label class="locSearch' . ($x % 2). '" for="locValue_' . $row[0] . '"><input name="locValue" id="locValue_' . $row[0] . '" type="radio" onclick="setLocation(' . $row[0] . ');" class="noBorderIE" />';
			...
			echo ($row[2] != '') ? ", " . htmlentities($row[4],ENT_QUOTES) : htmlentities($row[4],ENT_QUOTES);
			echo ($row[4] != '') ? ", " . htmlentities($row[5],ENT_QUOTES) : htmlentities($row[5],ENT_QUOTES);
			echo " " . htmlentities($row[6],ENT_QUOTES) . "</label><br />";
			++$x;
		}//end if
