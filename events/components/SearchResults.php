...
<?php	
	...
	if($city != ''){
		$query .= "	AND (l.IsActive = 1 OR l.IsActive is NULL)
					AND (e.LocationCity = '" . $city . "' OR l.City = '" . $city . "')";
...
?>	<b>Your Search Results</b> [<a href="<?php echo CalRoot;?>/index.php?com=searchresult&s=<?php echo urlencode($startDate);?>&e=<?php echo urlencode($endDate);?>&k=<?php echo urlencode($keyword) . $linkL . $linkC . $linkCat;?>" class="eventMain">Link to These Results</a>]<br>
	<?php
	...
	if(hasRows($result)){
		...
		<?php 	$calSaver = "";
			...
			<div class="<?php if($cnt % 2 == 0){echo "eventListTitle";}else{echo "eventListTitleHL";}?>"><a href="<?php echo CalRoot;?>/index.php?com=detail&amp;eID=<?php echo $row[0] . $calSaver;?>" class="eventListTitle" target="_blank"><?php echo cOut($row[1]);?></a></div>
<?php 	$cnt++;
		}//end while
	} else {	?>
		<br />
