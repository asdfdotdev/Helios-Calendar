...
*/
	...
	$endDateSQL = dateToMySQL(date($hc_cfg24,mktime(0,0,0,date("m"),date("d") + 14,date("Y"))), "/", $hc_cfg24);
	...
	echo "[<a href=\"" . CalRoot . "/index.php?com=searchresult&amp;s=" . urlencode($startDate) . "&amp;e=" . urlencode($endDate);?>&amp;k=<?php echo urlencode($keyword) . $linkL . $linkC . $linkP . $linkS . $linkCat . "\" class=\"eventMain\">" . $hc_lang_search['ResultLink'] . "</a>]";
	echo "<br />";
	...
	if(hasRows($result)){
		...
		while($row = mysql_fetch_row($result)){
			if(($curDate != $row[9]) or ($cnt == 0)){
				$curDate = $row[9];?>
				<div class="eventDateTitle"><?php echo stampToDate($row[9], $hc_cfg14);?></div>
		<?php 	$cnt = 0;
			}//end if?>
