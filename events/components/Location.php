		...
		<?php
				...
				if(hasRows($result)){
					...
					while($row = mysql_fetch_row($result)){
						$locID = cOut($row[0]);
						$resultLE = doQuery("SELECT * FROM " . HC_TblPrefix . "events WHERE IsActive = 1 AND IsApproved = 1 AND LocID = '" . $locID . "' AND StartDate >= NOW() ORDER BY StartDate LIMIT 15");
						if(hasRows($resultLE)){
							...
							$locOutput = "<div class=\"GmapLocMenu\"><a href=\"javascript:;\" onclick=\"toggleLoc(\'details" . $locID . "\',\'events" . $locID . "\');\" class=\"locMenu\">Details</a> | <a href=\"javascript:;\" onclick=\"toggleLoc(\'events" . $locID . "\',\'details" . $locID . "\');\" class=\"locMenu\">Events</a>&nbsp;|&nbsp;<a href=\"webcal://" . substr(CalRoot, 7) . "/link/SaveLocation.php?lID=" . $locID . "&amp;cID=1\" class=\"locMenu\">Subscribe (iCal)</a>&nbsp;&nbsp;<a href=\"" . CalRoot . "/rssL.php?lID=" . $locID . "\" target=\"_blank\"><img src=\"" . CalRoot . "/images/rss/feedIcon.gif\" width=\"16\" height=\"16\" alt=\"\" align=\"absmiddle\" /></a></div>";
