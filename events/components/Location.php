		...
		if(hasRows($result)){
			...
			while($row = mysql_fetch_row($result)){
				...
				if(hasRows($resultLE)){
					...
					$locTag .= buildAddress($locAddress,$locAddress2,$locCity,$locState,$locZip,$locCountry,$hc_lang_config['AddressType']);
					...
					while($rowLE = mysql_fetch_row($resultLE)){
						...
						$locEvents .= '<a href="' . CalRoot . '/index.php?com=detail&eID=' . $rowLE[0] . '" class="eventMain" target="_blank"><b>' . str_replace("'", "&#39;", htmlspecialchars($rowLE[1])) . '</b></a><br /><div class="hc_align">' . stampToDate($rowLE[9], $hc_cfg14) . '</div><div class="hc_align">&nbsp;-&nbsp;</div>' . $startTime . '<br /><br />';
					}//end while
					$locOutput = '<div class="GmapLocMenu"><a href="javascript:;" onclick="toggleLoc(\\\'details' . $locID . '\\\',\\\'events' . $locID . '\\\');" class="locMenu">' . str_replace("'", "&#39;", htmlspecialchars($hc_lang_locations['Details'])) . '</a><div class="hc_align">&nbsp;|&nbsp;</div><a href="javascript:;" onclick="toggleLoc(\\\'events' . $locID . '\\\',\\\'details' . $locID . '\\\');" class="locMenu">' . str_replace("'", "&#39;", htmlspecialchars($hc_lang_locations['Events'])) . '</a><div class="hc_align">&nbsp;|&nbsp;</div><a href="' . CalRoot . '/index.php?lID=' . $locID . '" class="locMenu">' . str_replace("'", "&#39;", htmlspecialchars($hc_lang_locations['Browse'])) . '</a><div class="hc_align">&nbsp;|&nbsp;</div><a href="webcal://' . substr(CalRoot, 7) . '/link/SaveLocation.php?lID=' . $locID . '" class="locMenu">' . str_replace("'", "&#39;", htmlspecialchars($hc_lang_locations['Subscribe'])) . '</a>&nbsp;&nbsp;<a href="' . CalRoot . '/rss/l.php?lID=' . $locID . '" target="_blank"><img src="' . CalRoot . '/images/rss/feedIcon.gif" width="16" height="16" alt="' . $locName . ' ' . $hc_lang_locations['FeedAlt'] . '" style="vertical-align:middle;" /></a></div>';
...
<?php		
			...
			if($linkList != ''){
				...
				echo "\n" . '<div id="locRest"><a href="javascript:;" onclick="goHome();" class="locListReset">' . $hc_lang_locations['Reset'] . '</a></div>';
