...
<?php 	if(in_array(2, $hc_captchas)){	?>
			if(document.frmSendToFriend.proof.value == ''){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_sendtofriend['Valid02'];?>';
			}//end if
<?php	}//end if	?>
...
<?php	
	...
	if(hasRows($result)){
		...
		<div align="right"><a href="<?php echo CalRoot;?>/?com=detail&amp;eID=<?php echo $eID;?>" class="eventMain">&laquo;&laquo; <?php echo $hc_lang_sendtofriend['ReturnLink'];?></a></div>
<?php	echo "<br />";
...
<?php	if(in_array(2, $hc_captchas)){	?>
		<fieldset>
			<legend><?php echo $hc_lang_sendtofriend['Authentication'];?></legend>
			<?php echo $hc_lang_sendtofriend['CannotRead'];?><br /><br />
			<div class="frmReq">
				<label for="proof">&nbsp;</label>
		<?php	buildCaptcha();	?><br />
			</div>
			<div class="frmReq">
				<label><?php echo $hc_lang_sendtofriend['ImageText'];?></label>
				...
				<div id="capChk"><a href="javascript:;" onclick="testCAPTCHA();" class="eventMain"><?php echo $hc_lang_sendtofriend['Confirm'];?></a></div>
			</div>
		</fieldset>
		<br />
	<?php
		...
		<fieldset>
			...
			<div class="frmReq">
				...
				<textarea name="hc_fx5" id="hc_fx5" rows="10" cols="45" onkeyup="this.value=this.value.slice(0, 250)"><?php echo $hc_lang_sendtofriend['SentMessage'];?></textarea>
			</div>
		</fieldset>
		<br />
