	...
	if(isset($_GET['com'])){
		switch ($_GET['com']){
			case 'detail':
				...
				$resultED = doQuery("SELECT * FROM " . HC_TblPrefix . "events WHERE IsActive = 1 AND IsApproved = 1 AND PkID = '" . $eID . "'");
				if(hasRows($resultED)){
					$hc_pTitle = html_entity_decode(cOut(strip_tags(mysql_result($resultED,0,1))));
					$hc_mDesc = substr(strip_tags(cOut(strip_tags(mysql_result($resultED,0,8)))),0,160);
			...
			case 'archive':
				if($hc_cfg54 == 1){
					...
					$hc_mDesc = $hc_md12;
					$hc_mKeys = $hc_mk12;
					$com = "components/NewsArchive.php";
				}//end if
				break;
			case 'signup':
				if($hc_cfg54 == 1){
			...
			case 'register':
				...
				break;
			case 'serieslist':
				$sID = (isset($_GET['sID'])) ? cIn($_GET['sID']) : 0;
			...
			case 'oacc':
				...
				$hc_mDesc = $hc_md16;
				$hc_mKeys = $hc_mk16;
				$com = (isset($_SESSION['hc_OpenIDLoggedIn'])) ? "components/OIDAccount.php" : $com;
				break;
			case 'ocomm':
				$hc_pTitle = $hc_pt16;
				$hc_mDesc = $hc_md16;
				$hc_mKeys = $hc_mk16;
				$com = (isset($_SESSION['hc_OpenIDLoggedIn'])) ? "components/OIDComments.php" : $com;
				break;
			case 'report':
				$hc_pTitle = $hc_pt17;
				$hc_mDesc = $hc_md17;
