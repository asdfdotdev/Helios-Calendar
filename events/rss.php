...
*/
	...
	hookDB();
	$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings WHERE PkID = 2");
  ...
  <channel>
	...
	<?php
		if(isset($_GET['cID'])){
			$result = doQuery("	SELECT distinct " . HC_TblPrefix . "events.*
								...
								WHERE " . HC_TblPrefix . "events.IsActive = 1 AND 
									...
									" . HC_TblPrefix . "categories.PkID IN ('" . $_GET['cID'] . "')
		...
		if($row_cnt > 0){
			...
			?>
				<item>
			      <title><?echo stampToDate($row[9], "m/d/Y") . " - " . str_replace('& ', '&amp; ', $row[1]);?></title>
			      <link><?echo CalRoot . "/index.php?com=detail&amp;eID=" . $row[0];?></link>
			      <description><?echo htmlspecialchars(str_replace('& ', '&amp; ', $row[8]));?></description>
			    </item>
			<?
			}//end while
		} else {
