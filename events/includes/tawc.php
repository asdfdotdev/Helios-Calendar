...
*/	
	...
	//	Do NOT edit any of the code below this point or you will likely cause your twitter updates to fail.
	
	include($hc_langPath . $_SESSION['LangSet'] . '/public/twitter.php');
	...
	$recentTweets = explode("/",$recentTweets);
	foreach($recentTweets as $val){
		unset($hc_lang_twitter[$val]);
	}//end if
	$abort = 0;
	...
	if($twtrMsg != '' && $abort != 1){
		...
		if(($fp = fsockopen($ip, $twPort, $errno, $errstr, 1)) ){
			...
			$request .= "User-Agent: Helios Calendar " . HC_Version . "\r\n";
