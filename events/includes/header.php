 ...
 */
	...
	if(!isset($_GET['com']) && isset($_GET['eID']) && is_numeric($_GET['eID'])){
		header("Location: index.php?com=detail&eID=" . $_GET['eID']);
	}//end if
	if(!isset($_SESSION['isMobile'])){
		include_once('agents.php');
		if(!preg_match($hc_mobile,$_SERVER['HTTP_USER_AGENT']) || $hc_cfg48 == 0){
			$_SESSION['isMobile'] = 0;
		} else {
			$reTarget = (isset($_GET['eID']) && is_numeric($_GET['eID'])) ? MobileRoot . '/details.php?eID=' . cIn($_GET['eID']) : MobileRoot;
			header("Location: " . $reTarget);
		}//
	}//end if
	...
	define('HC_Filter', 'components/FilterC.php');
	...
	if(isset($_GET['c']) && !isset($_GET['com'])){
		$cityNames = "";
		$cityName = explode(",", urldecode($_GET['c']));
	...
	if(!isset($_SESSION['hc_trail'])){$_SESSION['hc_trail'] = array();}
	...
	echo "\n\t" . '<meta name="description" content="' . str_replace("\"", "&quot;", $hc_mDesc) . '" />';
	...
	echo "\n\t" . '<!--[if lt IE 8]><link rel="stylesheet" type="text/css" href="' . CalRoot . '/css/heliosIE.css" /><![endif]-->';
	...
	echo "\n\n\t" . '<title>' . cOut($hc_pTitle) . ' - ' . CalName . ', powered by Helios Calendar</title>';?>