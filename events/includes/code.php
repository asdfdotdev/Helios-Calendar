...
*/
	...
	function makeTinyMCE($textName, $hc_WYSIWYG = 1, $width = "435px", $style = "simple", $passContent = ""){	?>
		<textarea name="<?php echo $textName;?>" id="<?php echo $textName;?>" style="width: <?php echo $width;?>; <?php if($hc_WYSIWYG == 0){echo "height: 200px;\"";}else{echo "height: 300px;\" id=\"goWYSIWYG\"";}?> rows="15" cols="55"><?php echo $passContent;?></textarea>
	<?php
		if($hc_WYSIWYG == 1){	?>
		<!-- tinyMCE -->
		...
		tinyMCE_GZ.init({
			plugins :  "spellchecker,advhr,advimage,advlink,contextmenu,emotions,fullscreen,inlinepopups,insertdatetime,layer,media,paste,preview,print,safari,searchreplace,style,table,xhtmlxtras",
			themes : "advanced",
			languages : "en",
			disk_cache : true,
			debug : false
	...
	<?php 	if($style == "advanced"){	?>
				...
				extended_valid_elements : "style[type],form[name|id|method|action|target],input[type|name|id|value|class|src],a[name|href|target|title|onclick],img[class|src|border=0|alt|title|hspace|vspace|width|height|align|onmouseover|onmouseout|name],hr[class|width|size|noshade],font[face|size|color|style],span[class|align|style],iframe[src|width|height|scrolling|border|marginwidth|style|frameborder],object[classid|codebase|width|height|align],param[name|value],embed[quality|type|pluginspage|width|height|src|align|wmode],script[language|src|type],",
	<?php 	} else {	?>
	...
	<?php 	}//end if	?>
				...
				document_base_url : "<?php echo CalRoot;?>",
				relative_urls : false,
...
<?php
	...
	function appInstructions($noSwitch, $codex, $title, $message){
		...
		<?php 	if($codex != ''){	?>
				<a href="http://www.refreshmy.com/documentation/index.php?title=<?php echo $codex;?>" class="main" target="_blank"><img src="<?php echo CalAdminRoot;?>/images/icons/iconHelp.png" width="16" height="16" alt="" border="0" /></a>
		<?php 	}//end if
...
<?php } else {	?>
			<div class="instructions" style="height: 16px;">
				...
				<div style="width:10%;float:left;text-align:right;">
		<?php 	if($codex != ''){	?>
				<a href="http://www.helioscalendar.com/documentation/index.php?title=<?php echo $codex;?>" class="main" target="_blank"><img src="<?php echo CalAdminRoot;?>/images/icons/iconHelp.png" width="16" height="16" alt="" border="0" /></a>
		<?php 	}//end if
...
<?php 	}//end if
	...
	?>	<a onmouseover="this.T_TITLE='<?php echo $title;?>';this.T_SHADOWCOLOR='#3D3F3E';return escape('<?php echo $message;?>')" href="javascript:;" class="eventMain"><img src="<?php echo CalAdminRoot;?>/images/icons/iconInfo.png" width="16" height="16" alt="" border="0" style="vertical-align:middle;" /></a><?php
...
<?php	if($cnt > 1){	?>
			...
			[ <a class="eventMain" href="javascript:;" onclick="checkAllArray('<?php echo $frmName;?>', 'catID[]');"><?php echo $hc_lang_core['SelectAll'];?></a> 
			&nbsp;|&nbsp; <a class="eventMain" href="javascript:;" onclick="uncheckAllArray('<?php echo $frmName;?>', 'catID[]');"><?php echo $hc_lang_core['DeselectAll'];?></a> ]
			</div>
<?php	}//end if
	}//end getCategories()
	...
	//	Clean International Characters
	function cleanIntChars($textToClean){
		$textToClean = str_replace("&Auml;", "A", $textToClean);
		$textToClean = str_replace("&auml;", "a", $textToClean);
		$textToClean = str_replace("&Aacute;", "A", $textToClean);
		$textToClean = str_replace("&aacute;", "a", $textToClean);
		$textToClean = str_replace("&Eacute;", "E", $textToClean);
		$textToClean = str_replace("&eacute;", "e", $textToClean);
		$textToClean = str_replace("&Egrave;", "E", $textToClean);
		$textToClean = str_replace("&egrave;", "e", $textToClean);
		$textToClean = str_replace("&Iacute;", "I", $textToClean);
		$textToClean = str_replace("&iacute;", "i", $textToClean);
		$textToClean = str_replace("&Ouml;", "O", $textToClean);
		$textToClean = str_replace("&ouml;", "o", $textToClean);
		$textToClean = str_replace("&Oacute;", "O", $textToClean);
		$textToClean = str_replace("&oacute;", "o", $textToClean);
		$textToClean = str_replace("&Uuml;", "U", $textToClean);
		$textToClean = str_replace("&uuml;", "u", $textToClean);
		$textToClean = str_replace("&Uacute;", "U", $textToClean);
		$textToClean = str_replace("&uacute;", "u", $textToClean);
		$textToClean = str_replace("&Ntilde;", "N", $textToClean);
		$textToClean = str_replace("&ntilde;", "n", $textToClean);
...
<?php
	...
	function buildCaptcha(){
		if(function_exists('imagecreate')){	?>
			<img src="<?php echo CalRoot;?>/includes/captcha.php" width="250" border="0" alt="" />
	<?php	
		} else {
