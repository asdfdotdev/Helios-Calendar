...
*/
	...
	function makeTinyMCE($textName, $hc_WYSIWYG = 1, $width = "435px", $style = "simple", $passContent = ""){	?>
		<textarea name="<?echo $textName;?>" id="<?echo $textName;?>" style="width: <?echo $width;?>; <?if($hc_WYSIWYG == 0){echo "height: 200px;\" convert_this=\"false\"";}else{echo "height: 300px;\"";}?>><?echo $passContent;?></textarea>
		<!-- tinyMCE -->
		...
		//<!--
			tinyMCE.init({
				...
				theme : "advanced",
				plugins : "advhr,advimage,advlink,contextmenu,emotions,insertdatetime,paste,preview,searchreplace,table,spellchecker",
		<?	if($style == "advanced"){	?>
				...
				theme_advanced_buttons3 : "tablecontrols,visualaid,separator,removeformat,hr,help,spellchecker",
...
<?	}//end makeHTMLArea()
	...
	function feedback($type, $fmessage){
		...
		<div class="<?echo $fType;?>">
			<img src="<?echo CalRoot;?>/images/feedback/<?echo $fIcon;?>" alt="" border="0" align="middle" />&nbsp;<?echo $fmessage;?>
		</div>
		<br />
<?	}//end feedback
	...
	function appInstructions($noSwitch, $codex, $title, $message){
		...
		?>	<div class="instructions">
			...
			<?	}//end if	?>
				...
				<div style="padding: 5px 0px 5px 10px;"><?echo $message;?></div>
			</div>
	<?	} else {	?>
...
<?php	if($cnt > 1){	?>
			...
			[ <a class="eventMain" href="javascript:;" onclick="checkAllArray('<?echo $frmName;?>', 'catID[]');">Select All Categories</a> 
			&nbsp;|&nbsp; <a class="eventMain" href="javascript:;" onclick="uncheckAllArray('<?echo $frmName;?>', 'catID[]');">Deselect All Categories</a> ]
			</div>
<?php	}//end if
	}//end getCategories()
	...
	function getCities($admin = 0){
		$sqlWhere = "";
		if($admin == 0){
			$sqlWhere = " AND e.StartDate >= NOW() ";
		}//end if
		$result = doQuery("SELECT e.LocationCity, l.City, l.IsActive
							FROM " . HC_TblPrefix . "events as e
								LEFT JOIN " . HC_TblPrefix . "locations as l ON (e.LocID = l.PkID)
							WHERE 
								(e.IsActive = 1 AND 
								e.IsApproved = 1 " . $sqlWhere . " )
	...
	function fakeFormFields(){	?>
		...
		<input name="phone" id="email" type="text" value="" style="display:none;" />
		<input name="message" id="email" type="text" value="" style="display:none;" />
<?php
	}//end fakeFormFields()	
	...
	function buildCaptcha(){
		if(function_exists('imagecreate')){	?>
			<img src="<?echo CalRoot;?>/includes/captcha.php" border="1" />
<?php	} else {
