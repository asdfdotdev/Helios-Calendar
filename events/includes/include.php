...
*/
	...
	if(!file_exists(realpath($incPrefix . 'cache/config.php'))){
		rebuildCache(0);
	}//end if
	include($incPrefix . 'cache/config.php');

	if(!isset($_SESSION[$hc_cfg00 . 'BrowseType'])){
		$_SESSION[$hc_cfg00 . 'BrowseType'] = $hc_cfg34;
	} else if(isset($_GET['b']) && is_numeric($_GET['b'])){
		$_SESSION[$hc_cfg00 . 'BrowseType'] = cIn($_GET['b']);
	}//end if
	...
	if(!isset($_SESSION[$hc_cfg00 . 'hc_favCat']) && isset($_COOKIE['hc_favCat'])){
		$_SESSION[$hc_cfg00 . 'hc_favCat'] = base64_decode($_COOKIE['hc_favCat']);
	}//end if
	
	if(!isset($_SESSION[$hc_cfg00 . 'hc_favCity']) && isset($_COOKIE['hc_favCity'])){
		$_SESSION[$hc_cfg00 . 'hc_favCity'] = base64_decode($_COOKIE['hc_favCity']);
	}//end if
	
	include($incPrefix . $hc_langPath . $_SESSION[$hc_cfg00 . 'LangSet'] . '/config.php');
