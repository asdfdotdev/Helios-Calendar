...
*/
	...
	if(!isset($_SESSION['license']) || $_SESSION['license'] == false){
		...
		if($stop == false && HC_Rando == ''){
			$stop = true;
			echo '<div class="error">HC_Rando String Required</div>';
		}//end if
		if($stop == false && strlen(HC_Rando) < 40){
			$warn = true;
			echo '<div class="warning">HC_Rando string length of at least 40 characters is recommended.</div>';
		}//end if
		if($stop == false){
		...
		if($stop == false && $warn == false){
			...
			echo '<div class="info">global.php configuration confirmed, READ THIS PAGE then click Continue below.</div>';
		}//end if
		...
		echo '<br /><br />';
		echo '<div style="float:left;width:170px;padding-left:25px;"><b>Admin Console:</b></div>';
		...
		echo '<br /><br />';
		echo '<div style="float:left;width:170px;padding-left:25px;"><b>HC_Rando String:</b></div>';
		echo (HC_Rando != '') ? '<div style="float:left;width:400px;">Set - ' . strlen(HC_Rando) . ' Characters</div>' : '<div style="float:left;width:400px;color:#DC143C;">Missing</div>';
