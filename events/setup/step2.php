...
*/
	...
	if(!isset($_SESSION['license']) || $_SESSION['license'] == false){
		...
		if($stop == false && !is_writable('../cache')){
			$stop = true;
			echo '<div class="error">Cache Directory Not Writable. Update Directory Permissions.</div>';
		}//end if
		if($stop == false && !is_writable('../includes/php-openid-2.1.2/_HeliosCalendar_consumers')){
			$stop = true;
			echo '<div class="error">OpenID Consumers Directory Not Writable. Update Directory Permissions.</div>';
		}//end if
