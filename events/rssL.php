...
<?php	$result = doQuery($query);
		...
		if(hasRows($result)){
			...
			while($row = mysql_fetch_row($result)){	
				...
				<item>
			      <title><![CDATA[ <?php echo cleanXMLChars(cleanIntChars(stampToDate(cOut($row[9]), $dateFormat))) . " - " . cleanXMLChars(cleanIntChars(cOut($row[1])));?> ]]></title>
			      <link><![CDATA[ <?php echo CalRoot . "/index.php?com=detail&eID=" . $row[0];?> ]]></link>
			      <description><![CDATA[ <?php echo htmlspecialchars_decode(cleanXMLChars(cleanIntChars(cOut($row[8]),$_SESSION['LangSet'])));?> ]]></description>
				  <guid><?php echo CalRoot . "/index.php&#63;com=detail&amp;eID=" . $row[0];?></guid>
				  <pubDate><?php echo cleanXMLChars(cleanIntChars(stampToDate(cOut($row[9]), "%a, %d %b %Y %H:%M:%S %z")));?></pubDate>
			    </item>
	<?php 		$cnt++;
			}//end while	
		} else {	?>
