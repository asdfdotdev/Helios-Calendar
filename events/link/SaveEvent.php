		...
		switch($cID){
			case 1:
				...
				if(mysql_result($result,0,11) == 0){
					...
					$link .= "&dates=" . stampToDate(mysql_result($result,0,9), "Ymd") . "/" . stampToDate(mysql_result($result,0,9), "%Y%m%d");
				}//end if
				
				$link .= "&text=" . urlencode(mysql_result($result,0,1));
				if(strlen(mysql_result($result,0,8)) > 500){
					$link .= "&details=" . urlencode(htmlspecialchars(strip_tags(cOut(substr(mysql_result($result,0,8),0,500)))) . "...<br /><br />Full Description available at: " . CalRoot . "/index.php?com=details&eID=" . $eID);
				} else {
			...
			case 2:
				...
				if(strlen(mysql_result($result,0,8)) > 500){
					$link .= "&desc=" . urlencode(strip_tags(substr(mysql_result($result,0,8),0,500) . "...\n\nFull Description available at: " . CalRoot . "/index.php?com=details&eID=" . $eID));
				} else {
