	...
	if(isset($_GET['tID']) && is_numeric($_GET['tID'])){
		if(isset($_GET['oID']) && is_numeric($_GET['oID'])){
			...
			//	Weather Link Clicked
				case 3:
						if($lID > 0){
							$result = doQUERY("SELECT Zip, City, State FROM " . HC_TblPrefix . "locations WHERE PkID = '" . cIn($_GET['lID']) . "'");
						} else {
						...
						if(hasRows($result)){
							...
							switch(mysql_result($resultSet,0,0)){
								...
								case 3:
									header('Location: http://weather.yahoo.com/forecast/' . urlencode(mysql_result($result,0,0)) . '.html');
									break;
								...
								case 7:
									header('Location: http://www.weatherzone.com.au/local/search.jsp?search=' . urlencode(mysql_result($result,0,1)) . '%20' . urlencode(mysql_result($result,0,0)));
									break;
							}//end switch
						} else {
