 ...
 */
	...
	$result = doQuery("SELECT PkID, Name, IsPublic FROM " . HC_TblPrefix  . "locations WHERE IsActive = 1 ORDER BY IsPublic, Name LIMIT " . $resLimit . " OFFSET " . ($resOffset * $resLimit));
	if(hasRows($result)){
		echo '
		...
		for($x = 25;$x <= 100;$x = $x + 25){
			echo ($x > 25) ? '&nbsp;|&nbsp;' : '';			
			echo ($x != $resLimit) ?
				'<a href="'.AdminRoot.'/index.php?com=location&amp;p='.$resOffset.'&amp;a='.$x.'">'.$x.'</a>':
				'<b>'.$x.'</b>';
		}
		...
		while($cnt <= ($resDiff * 2) && $x < $pages){
			echo ($cnt > 0) ? ' | ' : '';
			echo ($resOffset != $x) ?
				'<a href="'.AdminRoot.'/index.php?com=location&amp;p='.$x.'&amp;a='.$resLimit.'">'.($x + 1).'</a>':
				'<b>' . ($x + 1) . '</b>';
			++$x;
			++$cnt;
		}
		...
		function doDelete(dID){
			if(confirm("'.$hc_lang_locations['Valid01'].'\\n\\n          '.$hc_lang_locations['Valid02'].'\\n          '.$hc_lang_locations['Valid03'].'"))
				document.location.href = "'.AdminRoot.'/components/LocationEditAction.php?dID=" + dID;
		}
		//-->
		</script>';
	} else {
