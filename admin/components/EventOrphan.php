...
<?	if (isset($_GET['msg'])){
	...
	if(hasRows($result)){	
		...
		<form name="eventOrphan" id="eventOrphan" method="post" action="<?echo CalAdminRoot?>/components/EventDelete.php" onsubmit="return chkFrm();">
	...
	<?	$cnt = 0;
		while($row = mysql_fetch_row($result)){	?>
			...
			<input type="checkbox" name="eventID[]" id="eventID_<?echo $row[0];?>" value="<?echo $row[0];?>" class="noBorderIE" />&nbsp;</div>
		<?	$cnt++;
		}//end while	?>
		<br />&nbsp;&nbsp;<br />
		<div style="text-align:right;clear:both;">
