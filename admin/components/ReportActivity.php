...
*/
	...
	if(isset($_GET['print'])){
		...
		$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings WHERE PkID = 14");
...
<?php
	...
	if(hasRows($result)){
		...
		while($row = mysql_fetch_row($result)){
			...
			$eventDate = stampToDate($row[9], $hc_dateFormat);
			$publishDate = stampToDate($row[27], $hc_dateFormat);
			
			if(isset($print) && $cnt % 4 == 0){	
				if($cnt > 0){echo "<p style=\"page-break-before: always;\">&nbsp;</p>";}//end if?>
				<span class="main"><b><?php echo CalName . " " . $hc_lang_reports['EventReport'] . " -- " . $hc_lang_reports['PoweredBy'] . " Helios Calendar " . HC_Version;?></b></span>
