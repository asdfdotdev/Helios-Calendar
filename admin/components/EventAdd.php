...
<?php 	}//end if	?>
		...
		if(compareDates(document.frmEventAdd.eventDate.value, '<?php echo $hc_popDateValid;?>', '<?php echo date($hc_popDateFormat,mktime($hourOffset,date("i"),date("s"),date("m"),date("d")-1,date("Y")));?>', '<?php echo $hc_popDateValid;?>') == 0){
			if(!confirm('You have selected a past date for this event.\nAre you sure you want to add this event as a past event?\n\n          Ok = YES Continue Adding Past Event\n          Cancel = NO Cancel and Change Date')){
				return false;
			}//end if
		}//end if
	...
	function confirmRecurDates(){
		if(document.frmEventAdd.recurCheck.checked){
			...
			if(dirty > 0){
				...
				qStr = qStr + '&dateFormat=<?php echo $hc_popDateFormat;?>&eventDate=' + document.frmEventAdd.eventDate.value + '&recurEndDate=' + document.frmEventAdd.recurEndDate.value;
...
<?php
	...
	&nbsp; <span style="line-height: 23px;"><b>Add Event Using Live Clipboard</b></span>
	<br /><br />
	...
	<fieldset>
		...
		<div class="frmOpt">
				...
				<tr>
					...
					<td style="vertical-align: top;">
						...
						<div class="recPanel" id="monthly" style="display: none;">
							<input name="monthlyOption" id="monthlyOption1" type="radio" checked="checked" value="Day" class="noBorderIE" />Day&nbsp;<input name="monthlyDays" id="monthlyDays" type="text" maxlength="3" size="3" value="<?php echo date("d");?>" />&nbsp;of every&nbsp;<input name="monthlyMonths" id="monthlyMonths" type="text" maxlength="2" size="2" value="1" />&nbsp;month(s)<br />
							...
							<select name="monthlyMonthDOW" id="monthlyMonthDOW">
								<option <?php if(date("w") == 0){echo "selected=\"seclected\"";}?> value="0">Sunday</option>
								<option <?php if(date("w") == 1){echo "selected=\"seclected\"";}?> value="1">Monday</option>
								<option <?php if(date("w") == 2){echo "selected=\"seclected\"";}?> value="2">Tuesday</option>
								<option <?php if(date("w") == 3){echo "selected=\"seclected\"";}?> value="3">Wednesday</option>
								<option <?php if(date("w") == 4){echo "selected=\"seclected\"";}?> value="4">Thursday</option>
								<option <?php if(date("w") == 5){echo "selected=\"seclected\"";}?> value="5">Friday</option>
								<option <?php if(date("w") == 6){echo "selected=\"seclected\"";}?> value="6">Saturday</option>
							</select>
			...
			<div class="frmOpt">
				...
				<input name="recurEndDate" id="recurEndDate" type="text" value="" disabled="disabled" size="10" maxlength="10" />&nbsp;<a href="javascript:;" onclick="calx.select(document.frmEventAdd.recurEndDate,'anchor2','<?php echo $hc_popDateValid;?>'); return false;" name="anchor2" id="anchor2"><img src="<?php echo CalRoot;?>/images/icons/iconCalendar.gif" width="16" height="16" border="0" alt="" /></a>
			</div>
			<br />
			<label>&nbsp;</label>
			<div id="recurDateChk">
