 ...
 */
	...
	header("Location: " . CalAdminRoot . "/index.php?com=generalset");
	exit();}
	...
	$mailUser = ($mailAuth == 1) ? $_POST['mailUser'] : '';
	$mailPassChg = ($mailAuth == 1) ? $_POST['mailPassChg'] : '';
	$mailPassCon = ($mailAuth == 1) ? $_POST['mailPassCon'] : '';
	$mailAddress = $_POST['mailAddress'];
	$mailName = $_POST['mailName'];
	$loginTries = $_POST['loginTries'];
	$pubNews = $_POST['pubNews'];
	$batchSize = $_POST['batchSize'];
	$batchDelay = $_POST['batchDelay'];

	if($capType > 0){
		if(isset($_POST['capID'])){
			$capIDs = implode(',',$_POST['capID']);
		}//end if
		if($capType == 2){
	...
	if($mailPassChg != '' && $mailPassChg == $mailPassCon && $mailAuth == 1){
		doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . base64_encode(cIn($mailPassChg)) . "' WHERE PkID = 76");
	...
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($pubNews) . "' WHERE PkID = 54");
	...
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailMethod) . "' WHERE PkID = 71");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailHost) . "' WHERE PkID = 72");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailPort) . "' WHERE PkID = 73");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailSecure) . "' WHERE PkID = 74");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailUser) . "' WHERE PkID = 75");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailAuth) . "' WHERE PkID = 77");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailAddress) . "' WHERE PkID = 78");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($mailName) . "' WHERE PkID = 79");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($loginTries) . "' WHERE PkID = 80");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($batchSize) . "' WHERE PkID = 81");
	doQuery("UPDATE " . HC_TblPrefix . "settings SET SettingValue = '" . cIn($batchDelay) . "' WHERE PkID = 82");
	...
	header("Location: " . CalAdminRoot . "/index.php?com=generalset&msg=" . $msgID);?>