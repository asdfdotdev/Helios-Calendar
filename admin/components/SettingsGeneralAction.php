 ...
 */
	...
	$msgID = 1;
	$reCapPub = $reCapPriv = '';
	$maxDisplay = isset($_POST['maxRSS']) ? cIn($_POST['maxRSS']) : '';
	...
	$iCalMax = isset($_POST['iCalMax']) ? cIn($_POST['iCalMax']) : '';
	$iCalRef = isset($_POST['iCalRef']) ? cIn($_POST['iCalRef']) : '';
	...
	if($allowsubmit != ''){
		...
		if($allowsubmit == 1){
			$pubCat = cIn(strip_tags($_POST['pubCat']));
			$subLimit = cIn(strip_tags($_POST['subLimit']));
