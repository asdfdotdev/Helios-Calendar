...
*/
	...
	$resultTemplate = doQuery("SELECT TemplateSource FROM " . HC_TblPrefix . "newsletters WHERE PkID = " . $_POST['templateID']);
	
	$message = mysql_result($resultTemplate,0,0);
	...
	while($row = mysql_fetch_row($result)){
		...
		$resultEvents = doQuery("	SELECT " . HC_TblPrefix . "events.PkID, " . HC_TblPrefix . "events.Title, " . HC_TblPrefix . "events.StartDate, " . HC_TblPrefix . "events.SeriesID 
									...
									WHERE 
										" . HC_TblPrefix . "usercategories.UserID = " . $row[0] . " AND " . HC_TblPrefix . "events.StartDate Between '" . $startDate . "' AND '" . $endDate . "' 
	...
	function getBillboard($type){
		...
		if(hasRows($resultBillboard)){
			while($row = mysql_fetch_row($resultBillboard)){
				$billboard .= "<li><a href=\"" . CalRoot . "/index.php?com=detail&eID=" . $row[0] . "\" target=\"new\">" . $row[1] . "</a>";
			}//end while
		} else {
