 ...
 */
	...
	$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings 
					WHERE PkID IN (5,6,25,27,36,37,38,39,41,42,43,45,46,47,50,52,55,56,57,58,59,62,63,69,94,95,96,
	...
	$mapLibrary = cOut(mysql_result($result,16,0));
	$useComments = cOut(mysql_result($result,17,0));
	$bitlyUser = cOut(mysql_result($result,18,0));
     $bitlyAPI = cOut(mysql_result($result,19,0));
	$tweetHash = cOut(mysql_result($result,20,0));
	$eventbriteOrgID = cOut(mysql_result($result,21,0));
	$twtrUsername = cOut(mysql_result($result,22,0));
	$lmapLat = ($locBrowse > 0) ? cOut(mysql_result($result,9,0)) : '';
	$lmapLon = ($locBrowse > 0) ? cOut(mysql_result($result,10,0)) : '';
	$mapSyndication = ($locBrowse > 0) ? cOut(mysql_result($result,23,0)) : 0;
	$mapLayer = cOut(mysql_result($result,24,0));
	$yahooAPI = cOut(mysql_result($result,25,0));
	$bingAPI = cOut(mysql_result($result,26,0));
	$fbcPosts = cOut(mysql_result($result,27,0));
	$fbcWidth = cOut(mysql_result($result,28,0));
	$livefyreID = cOut(mysql_result($result,29,0));
	$eventbritePaypal = cOut(mysql_result($result,30,0));
	$eventbriteGoogleID = cOut(mysql_result($result,31,0));
	$eventbriteGoogleKey = cOut(mysql_result($result,32,0));
	$twtrComKey = cOut(mysql_result($result,33,0));
	$twtrComSec = cOut(mysql_result($result,34,0));
	$twtrSignIn = cOut(mysql_result($result,35,0));
	$fbSignIn = cOut(mysql_result($result,36,0));
	$googSignIn = cOut(mysql_result($result,37,0));
	$fbAppID = cOut(mysql_result($result,38,0));
	$fbAppSec = cOut(mysql_result($result,39,0));	
	$fbConOpts = json_decode(cOut(mysql_result($result,40,0)),true);
	$fbActiveT = cOut(mysql_result($result,41,0));
	$fbTokenU = cOut(mysql_result($result,42,0));
	$fbExpires = cOut(mysql_result($result,43,0));
	$fbActiveI = cOut(mysql_result($result,44,0));
	$googID = cOut(mysql_result($result,45,0));
	$googSec = cOut(mysql_result($result,46,0));
	$passOutput = $fbConOptsOut = '';
