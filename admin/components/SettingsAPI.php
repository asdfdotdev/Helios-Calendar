 ...
 */
	...
	$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings 
					...
					ORDER BY PkID");
	$eventbriteKeyA = cOut(Amysqlresult($result,0,0));
	$eventbriteKeyU = cOut(Amysqlresult($result,1,0));
	$disqusName = cOut(Amysqlresult($result,2,0));
	$emapZoom = cOut(Amysqlresult($result,3,0));
	$lmapZoom = cOut(Amysqlresult($result,4,0));
	$locBrowse = cOut(Amysqlresult($result,7,0));
	$twtrAToken = cOut(Amysqlresult($result,8,0));
	$twtrASecret = cOut(Amysqlresult($result,9,0));
	$quickLinks = explode(",", cOut(Amysqlresult($result,10,0)));
	$googMapURL = cOut(Amysqlresult($result,11,0));
	$mapLibrary = cOut(Amysqlresult($result,12,0));
	$useComments = cOut(Amysqlresult($result,13,0));
	$bitlyUser = cOut(Amysqlresult($result,14,0));
     $bitlyAPI = cOut(Amysqlresult($result,15,0));
	$tweetHash = cOut(Amysqlresult($result,16,0));
	$googMapVer = cOut(Amysqlresult($result,17,0));
	$eventbriteOrgID = cOut(Amysqlresult($result,18,0));
	$twtrUsername = cOut(Amysqlresult($result,19,0));
	$lmapLat = ($locBrowse > 0) ? cOut(Amysqlresult($result,5,0)) : '';
	$lmapLon = ($locBrowse > 0) ? cOut(Amysqlresult($result,6,0)) : '';
	$mapSyndication = ($locBrowse > 0) ? cOut(Amysqlresult($result,20,0)) : 0;
	$mapLayer = cOut(Amysqlresult($result,21,0));
	$yahooAPI = cOut(Amysqlresult($result,22,0));
	$bingAPI = cOut(Amysqlresult($result,23,0));
	$fbcPosts = cOut(Amysqlresult($result,24,0));
	$fbcWidth = cOut(Amysqlresult($result,25,0));
	$livefyreID = cOut(Amysqlresult($result,26,0));
	$eventbritePaypal = cOut(Amysqlresult($result,27,0));
	$eventbriteGoogleID = cOut(Amysqlresult($result,28,0));
	$eventbriteGoogleKey = cOut(Amysqlresult($result,29,0));
	$twtrComKey = cOut(Amysqlresult($result,30,0));
	$twtrComSec = cOut(Amysqlresult($result,31,0));
	$twtrSignIn = cOut(Amysqlresult($result,32,0));
	$fbSignIn = cOut(Amysqlresult($result,33,0));
	$googSignIn = cOut(Amysqlresult($result,34,0));
	$fbAppID = cOut(Amysqlresult($result,35,0));
	$fbAppSec = cOut(Amysqlresult($result,36,0));	
	$fbConOpts = json_decode(cOut(Amysqlresult($result,37,0)),true);
	$fbActiveT = cOut(Amysqlresult($result,38,0));
	$fbTokenU = cOut(Amysqlresult($result,39,0));
	$fbExpires = cOut(Amysqlresult($result,40,0));
	$fbActiveI = cOut(Amysqlresult($result,41,0));
	$googID = cOut(Amysqlresult($result,42,0));
	$googSec = cOut(Amysqlresult($result,43,0));
	$fbConOptsOut = '';
