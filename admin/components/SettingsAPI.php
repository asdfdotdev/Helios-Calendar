 ...
 */
	...
	$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings 
					...
					ORDER BY PkID");
	$eventbriteKeyA = cOut(hc_mysql_result($result,0,0));
	$eventbriteKeyU = cOut(hc_mysql_result($result,1,0));
	$disqusName = cOut(hc_mysql_result($result,2,0));
	$emapZoom = cOut(hc_mysql_result($result,3,0));
	$lmapZoom = cOut(hc_mysql_result($result,4,0));
	$locBrowse = cOut(hc_mysql_result($result,7,0));
	$twtrAToken = cOut(hc_mysql_result($result,8,0));
	$twtrASecret = cOut(hc_mysql_result($result,9,0));
	$quickLinks = explode(",", cOut(hc_mysql_result($result,10,0)));
	$googMapURL = cOut(hc_mysql_result($result,11,0));
	$mapLibrary = cOut(hc_mysql_result($result,12,0));
	$useComments = cOut(hc_mysql_result($result,13,0));
	$bitlyUser = cOut(hc_mysql_result($result,14,0));
     $bitlyAPI = cOut(hc_mysql_result($result,15,0));
	$tweetHash = cOut(hc_mysql_result($result,16,0));
	$googMapVer = cOut(hc_mysql_result($result,17,0));
	$eventbriteOrgID = cOut(hc_mysql_result($result,18,0));
	$twtrUsername = cOut(hc_mysql_result($result,19,0));
	$lmapLat = ($locBrowse > 0) ? cOut(hc_mysql_result($result,5,0)) : '';
	$lmapLon = ($locBrowse > 0) ? cOut(hc_mysql_result($result,6,0)) : '';
	$mapSyndication = ($locBrowse > 0) ? cOut(hc_mysql_result($result,20,0)) : 0;
	$mapLayer = cOut(hc_mysql_result($result,21,0));
	$yahooAPI = cOut(hc_mysql_result($result,22,0));
	$bingAPI = cOut(hc_mysql_result($result,23,0));
	$fbcPosts = cOut(hc_mysql_result($result,24,0));
	$fbcWidth = cOut(hc_mysql_result($result,25,0));
	$livefyreID = cOut(hc_mysql_result($result,26,0));
	$eventbritePaypal = cOut(hc_mysql_result($result,27,0));
	$eventbriteGoogleID = cOut(hc_mysql_result($result,28,0));
	$eventbriteGoogleKey = cOut(hc_mysql_result($result,29,0));
	$twtrComKey = cOut(hc_mysql_result($result,30,0));
	$twtrComSec = cOut(hc_mysql_result($result,31,0));
	$twtrSignIn = cOut(hc_mysql_result($result,32,0));
	$fbSignIn = cOut(hc_mysql_result($result,33,0));
	$googSignIn = cOut(hc_mysql_result($result,34,0));
	$fbAppID = cOut(hc_mysql_result($result,35,0));
	$fbAppSec = cOut(hc_mysql_result($result,36,0));
	$fbConOpts = json_decode(cOut(hc_mysql_result($result,37,0)),true);
	$fbActiveT = cOut(hc_mysql_result($result,38,0));
	$fbTokenU = cOut(hc_mysql_result($result,39,0));
	$fbExpires = cOut(hc_mysql_result($result,40,0));
	$fbActiveI = cOut(hc_mysql_result($result,41,0));
	$googID = cOut(hc_mysql_result($result,42,0));
	$googSec = cOut(hc_mysql_result($result,43,0));
	$fbConOptsOut = '';
