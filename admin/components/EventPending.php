 ...
 */
	...
	if(!isset($_GET['sID']) && !isset($_GET['eID'])){
		$result = doQuery("SELECT PkID, Title, StartDate, SeriesID
							...
							WHERE IsActive = 1 AND IsApproved = 2
		...
		if(hasRows($result)){
			...
			while($row = mysql_fetch_row($result)){
				...
				<li class="row header uline">
					<div style="width:100%;">'.$hc_lang_event['PendingIndividual'].'</div>
				</li>' : '';
				...
				if($row[3] != '' && $curSeries != $row[3]){
					...
					echo '
				<li class="row header uline">
					<div style="width:90%;">'.$hc_lang_event['PendingSeries'].'</div>
				...
				echo '
				<li class="row'.$hl.'">
					<div class="text" style="width:75%;">'.cOut($row[1]).'</div>
					<div class="text" style="width:15%;">'.stampToDate($row[2], $hc_cfg[24]).'</div>
					<div class="tools" style="width:10%;">
		...
		$query = "SELECT e.*, l.PkID, l.Name, l.Address, l.Address2, l.City, l.State, l.Zip, l.Country
		...
		if(!hasRows($result)){
			...
			'.(($message != '') ? '
			<fieldset>
				<legend>'.$hc_lang_event['Message'].'</legend>
				<p>'.nl2br($message).'</p>
			</fieldset>':'').'
			<fieldset>
			...
			echo '
				...
				<div id="custom" style="'.(($locID > 0) ? 'display:none;' : '').'clear:both;padding:15px 0 0 0;">
					<label>&nbsp;</label>
					<span class="frm_ctrls">
						<label for="newLoc"><input name="newLoc" id="newLoc" type="checkbox" checked="checked" />'.$hc_lang_event['LocNew'].'</label>
					</span>
			...
			<fieldset>
				...
				<p>
					'.(($efID != '' || $ebID != '') ? '<b>'.$hc_lang_event['PostedTo'].'</b>' : '').'
				...
				<p><i>' . $hc_lang_event['DistPubNotice'] . '</i></p>
			...
			<fieldset>
				...
				<span class="frm_ctrls">
					<label for="sendmsg"><input name="sendmsg" id="sendmsg" type="checkbox" onclick="javascript:chgButton();">'.$hc_lang_event['SendMsg'].' <b>'.$subName.'</b> (<a href="mailto:'.$subEmail.'">'.$subEmail.'</a>)</label>
				</span>
				...
				<textarea rows="20" name="message" id="message">'.$emailAccept.'</textarea>' : '
