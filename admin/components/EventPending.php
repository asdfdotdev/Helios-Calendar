	...
	if(!isset($_GET['sID']) && !isset($_GET['eID'])){
		...
		if(hasRows($resultI) || hasRows($resultS)){
			...
			function deleteSeries(sID){
				if(confDelete())
					document.location.href = "'.AdminRoot.'/components/EventDelete.php?series=" + sID + "&pID=1";
			}
			function confDelete(){
		...
		$query = "SELECT e.*, l.PkID, l.Name, l.Address, l.Address2, l.City, l.State, l.Zip, l.Country, f.EntityID, f.Note, u.NetworkName, u.Email
				FROM " . HC_TblPrefix . "events e
		...
		if(!hasRows($result)){
			...
			$followup = (mysql_result($result,0,50) != '') ? 1 : 0;
			$fnote = cOut(mysql_result($result,0,51));
			...
			$subName = (mysql_result($result,0,41) > 0) ? cOut(mysql_result($result,0,52)) : cOut(mysql_result($result,0,20));
			$subEmail = (mysql_result($result,0,41) > 0) ? cOut(mysql_result($result,0,53)) : cOut(mysql_result($result,0,21));
			...
			$result = doQuery("SELECT * FROM " . HC_TblPrefix . "settings WHERE PkID IN(5,6,36,37,38,39,46,47,57,58,120,123)");
			...
			$goFacebook = (mysql_result($result,10,1) != '' && mysql_result($result,11,1) != '') ? 1 : 0;
			$efID = $ebID = $tweetLnks = $fbID = $fbStatLnks = '';
			...
			<fieldset>
				...
				<div id="facebook" style="display:none;">
					...
					<label for="facebookEvent" class="distPubFB"><input name="facebookEvent" id="facebookEvent" type="checkbox" onclick="toggleMe(document.getElementById(\'facebookEventLabel\'));" /> '.(($fbID == '') ? $hc_lang_event['FacebookLabelA']:$hc_lang_event['FacebookEventU']).'</label>
				...
				$myTZ = substr(HCTZ,0,3);
				for($x = -12; $x <= 14; ++$x){
					$tzs = ($x < 0) ? '-' : '+';
					$tz = ($x < 10 && $x > -10) ? '0'.abs($x) : abs($x);
					$tz = $tzs.$tz;
					echo '
						<option'.(($x == $myTZ) ? ' selected="selected"':'').' value="GMT'.$tz.'">GMT'.$tz.'</option>';
				}
				echo '
					</select>
				...
				<label for="doEventful" class="distPub">
			...
			<fieldset>
				...
				<textarea rows="20" name="message" id="message">'.cOut($emailAccept).'</textarea>' : '
			...
			makeTinyMCE('eventDescription,message',1,0,0);
 		}
	}
?>