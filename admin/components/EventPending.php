...
*/
	...
	<?php 	while($row = mysql_fetch_row($result)){
				if($row[3] == '' && $cnt == 0){	?>  
					<div class="pendingList"><?php echo $hc_lang_event['PendingIndividual'];?></div>
		<?php 	}//end if
				...
				if($row[3] != '' && $curSeries != $row[3]){
					...
					<div class="pendingList">
						<div style="width: 450px; float:left;"><?php echo $hc_lang_event['PendingSeries'];?></div>
						<div style="width: 73px;float:left;text-align:right;"><a href="<?php echo CalAdminRoot;?>/index.php?com=eventpending&amp;sID=<?php echo $row[3];?>" class="main"><img src="<?php echo CalAdminRoot;?>/images/icons/iconEditGroup.png" width="15" height="15" alt="" border="0" style="vertical-align:middle;" /></a></div>
						&nbsp;
					</div>
		<?php 	}//end if	?>
					...
					<div class="pendingListTools<?php if($cnt % 2 == 1){echo "HL";}?>">&nbsp;<a href="<?php echo CalAdminRoot;?>/index.php?com=eventpending&amp;eID=<?php echo $row[0];?>" class="main"><img src="<?php echo CalAdminRoot;?>/images/icons/iconEdit.png" width="15" height="15" alt="" border="0" style="vertical-align:middle;" /></a>
...
<?php 	} else {	?>
			<br />
			<b><?php echo $hc_lang_event['NoPending'];?></b>
<?php 	}//end if
	} else {
		$hrFormat = "h";
		$minHr = 1;
	...
	<?php 	}//end if	?>
			...
			if(compareDates(document.frmEventApprove.eventDate.value, '<?php echo $hc_popDateValid;?>', '<?php echo strftime($hc_popDateFormat,mktime($hourOffset,date("i"),date("s"),date("m"),date("d")-1,date("Y")));?>', '<?php echo $hc_popDateValid;?>') == 0){
		...
		function searchLocations($page){
			if(document.frmEventApprove.locSearch.value.length > 3){
				var qStr = 'LocationSearch.php?q=' + escape(document.frmEventApprove.locSearch.value) + '&o=' + $page;
	...
	<?php 	}//end if	?>
		...
		<?php 	}//end if	?>
				<div class="frmOpt">
					...
					<input name="cost" id="cost" type="text" value="<?php echo $cost;?>" size="15" maxlength="50" />
				</div>
			</fieldset>
			<br />
			<fieldset>
		...
		<?php 	if($subEmail != ''){	?>
			...
			<?php	echo $subName;?> (<a href="mailto:<?php echo $subEmail;?>" class="main"><?php echo $subEmail;?></a>),
					<textarea disabled="disabled" rows="7" cols="60" name="message" id="message"><?php echo $emailAccept;?></textarea><br />
					<label>&nbsp;</label>
