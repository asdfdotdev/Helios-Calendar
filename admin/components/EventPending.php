...
<?php 	} else {
		...
		$emailAccept = cleanBreaks(mysql_result($result,0,0));
		$emailDecline = cleanBreaks(mysql_result($result,1,0));
		...
		warn = "<?php echo $hc_lang_event['Valid01b'];?>";
			...
			if(document.getElementById('eventStatus').value == 0){
				return true;
			}//end if
			
			if(document.getElementById('eventRegistration').value == 1){
				if(isNaN(document.getElementById('eventRegAvailable').value) == true){
					dirty = 1;
		...
		<?php
			if(!isset($_GET['sID'])){	?>
			if(!isDate(document.getElementById('eventDate').value, '<?php echo $hc_cfg51;?>')){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid24'] . " " . strtolower($hc_cfg51);?>';
			}//end if
			
			if(isNaN(document.getElementById('startTimeHour').value) == true){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid05'];?>';
			} else if((document.getElementById('startTimeHour').value > <?php echo $hc_timeInput;?>) || (document.getElementById('startTimeHour').value < 1)) {
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid06'] . " " . $minHr . " - " . $hc_timeInput;?>';
			}//end if
			
			if(isNaN(document.getElementById('startTimeMins').value) == true){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid07'];?>';
			} else if((document.getElementById('startTimeMins').value > 59) || (document.getElementById('startTimeMins').value < 0)) {
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid08'];?> 0 - 59';
			}//end if
			
			if(isNaN(document.getElementById('endTimeHour').value) == true){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid09'];?>';
			} else if((document.getElementById('endTimeHour').value > <?php echo $hc_timeInput;?>) || (document.getElementById('endTimeHour').value < 1)) {
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid10'] . " " . $minHr . " - " . $hc_timeInput;?>';
			}//end if
			
			if(isNaN(document.getElementById('endTimeMins').value) == true){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid11'];?>';
			} else if((document.getElementById('endTimeMins').value > 59) || (document.getElementById('endTimeMins').value < 0)) {
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_event['Valid12'];?> 0 - 59';
			}//end if

			if(compareDates(document.getElementById('eventDate').value, '<?php echo $hc_cfg51;?>', '<?php echo strftime($hc_cfg24,mktime($hourOffset,date("i"),date("s"),date("m"),date("d")-1,date("Y")));?>', '<?php echo $hc_cfg51;?>') == 0){
	...
	<?php 	}//end if	?>
			...
			if(document.getElementById('contactEmail').value != '' && chkEmail(document.getElementById('contactEmail')) == 0){
				dirty = 1;
		...
		function chgStatus(){
			if(document.getElementById('eventStatus').value == 1){
				document.getElementById('message').value = "<?php echo $emailAccept;?>";
			} else if(document.getElementById('eventStatus').value == 0) {
				document.getElementById('message').value = "<?php echo $emailDecline;?>";
			} else {
		...
		function chgButton(){
			if(document.getElementById('sendmsg').checked){
				...
				document.getElementById('submit').value = ' <?php echo htmlentities($hc_lang_event['SaveWMessage'],ENT_QUOTES);?>';
			} else {
				document.getElementById('message').disabled = true;
				document.getElementById('submit').value = ' <?php echo htmlentities($hc_lang_event['SaveWOMessage'],ENT_QUOTES);?> ';
			}//end if
		}//end chgButton()
	...
	<?php 	}//end if	?>
		...
		<?php 	if($editType == 1){	?>
				...
				<div class="frmOpt">
					...
					<div class="eventStartCtrl"><a href="javascript:;" onclick="chngClock(document.getElementById('startTimeMins'),5,59)"><img src="<?php echo CalRoot;?>/images/time_up.gif" width="16" height="8" alt="<?php echo $hc_lang_event['ALTIncStartM'];?>" /></a><br /><a href="javascript:;" onclick="chngClock(document.getElementById('startTimeMins'),-5,59)"><img src="<?php echo CalRoot;?>/images/time_down.gif" width="16" height="9" alt="<?php echo $hc_lang_event['ALTDecStartM'];?>" /></a></div>
			...
			<?php 	}//end if	?>
				...
				<div class="frmOpt">
					...
					<div class="eventEndCtrl"><a href="javascript:;" onclick="chngClock(document.getElementById('endTimeMins'),5,59)"><img src="<?php echo CalRoot;?>/images/time_up.gif" width="16" height="8" alt="<?php echo $hc_lang_event['ALTIncEndM'];?>" /></a><br /><a href="javascript:;" onclick="chngClock(document.getElementById('endTimeMins'),-5,59)"><img src="<?php echo CalRoot;?>/images/time_down.gif" width="16" height="9" alt="<?php echo $hc_lang_event['ALTDecEndM'];?>" /></a></div>
			<?php 	if($hc_timeInput == 12){	?>
		...
		<?php		$query = "SELECT c.PkID, c.CategoryName, c.ParentID, c.CategoryName as Sort, ec.EventID as Selected
			...
			<?php
				...
				echo ($goTwitter == 1 && $goBitly == 1) ?  ' />&nbsp;' : ' disabled="disabled" />&nbsp;';
				...
				echo '<div class="frmOpt" style="border-top:solid 1px #CCCCCC;"><label for="doBitly" class="radioDistPub"><input name="doBitly" id="doBitly" type="checkbox" onclick="toggleMe(document.getElementById(\'bitly\'));" class="noBorderIE"';
				echo (strpos($shortURL,"http://") !== false) ? ' checked="checked"' : '';
				echo ($goBitly == 1 && strpos($shortURL,"http://") === false) ?  ' />&nbsp;' : ' disabled="disabled" />&nbsp;';

				if(strpos($shortURL,"http://") === false){
					echo $hc_lang_event['BitlyLabel'] . '</label></div>';
					echo '<div id="bitly" style="display:none;clear:both;padding:7px 5px 0px 5px;">' . $hc_lang_event['BitlyNotice'] . '</div>';
				} else {
					echo $hc_lang_event['BitlyHasLink'] . '</label></div>';
					echo '<div id="bitly" style="clear:both;padding:7px 5px 0px 5px;">' . $hc_lang_event['BitlyTools'];
					echo '<ul>';
					echo '<li><a href="' . $shortURL . '+" class="main" target="_blank">' . $hc_lang_event['BitlyReport'] . '</a></li>';
					echo '<li><a href="' . $shortURL . '" class="main" target="_blank">' . $hc_lang_event['BitlyLink'] . '</a></li>';
					echo '<li><a href="' . $shortURL . '.qrcode" class="main" target="_blank">' . $hc_lang_event['BitlyQRImage'] . '</a></li>';
					echo '<li><a href="' . $shortURL . '.qr" class="main" target="_blank">' . $hc_lang_event['BitlyQRLink'] . '</a></li>';
					echo '</ul></div>';
				}//end if?>
			</fieldset>
			<br />
		...
		<?php 	if($subEmail != ''){	?>
				<div class="frmOpt">
					<label for="sendmsg" class="radioWide" style="width:100%;"><input name="sendmsg" id="sendmsg" type="checkbox" onclick="javascript:chgButton();" class="noBorderIE" /> <?php echo $hc_lang_event['SendMsg'] . ' ' . $subName . ' (' . $subEmail . ')';?></label>
				</div>
				<br /><br />
				<div class="frmOpt">
