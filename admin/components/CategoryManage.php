...
<?php
	...
	<fieldset style="padding-top:5px;">
		<legend><?php echo $where;?> Category<?php if($cID > 0){?>&nbsp;&nbsp;( <a href="<?php echo CalAdminRoot;?>/index.php?com=categorymanage" class="main">Add New</a> )<?php }?></legend>
		<div class="frmReq">
			<label>Name:</label>
...
<?php 	$result = doQuery("SELECT PkID, CategoryName FROM " . HC_TblPrefix . "categories WHERE IsActive = 1 ORDER BY CategoryName");
		...
		if(hasRows($result)){	
			...
			while($row = mysql_fetch_row($result)){
				...
				<div class="categoryListTools<?php if($cnt % 2 == 1){echo "HL";}?>"><a href="<?php echo CalAdminRoot;?>/index.php?com=categorymanage&amp;cID=<?php echo $row[0];?>" class="main"><img src="<?php echo CalAdminRoot;?>/images/icons/iconEdit.gif" width="15" height="15" alt="" border="0" align="absmiddle" /></a><?php if($rowCnt > 1){?>&nbsp;<a href="javascript:;" onclick="doDelete('<?php echo $row[0];?>');return false;" class="main"><img src="<?php echo CalAdminRoot;?>/images/icons/iconDelete.gif" width="15" height="15" alt="" border="0" align="absmiddle" /></a><?php }//end if?></div>
		<?php 	$curCat = $row[0];
				$cnt++;
				}//end if
