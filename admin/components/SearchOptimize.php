...
<?php
	...
	while($row = mysql_fetch_row($result)){
		echo '<input type="hidden" name="ids[]" value="' . $row[0] . '" />';
		echo '<fieldset>';
		echo '<legend>' . $hc_lang_settings['Page' . $row[0]] . '</legend>';
		echo '<div class="frmOpt">';
		echo '<label for="pgtitle' . $row[0] . '">' . $hc_lang_settings['Title'] . '</label>';
		echo '<input name="titles[]" id="pgtitle' . $row[0] . '" size="65" maxlength="65" type="text" value="' . cOut($row[3]) . '" />';
		echo '</div>';
		echo '<div class="frmOpt">';
		echo '<label for="keywords' . $row[0] . '">' . $hc_lang_settings['Keywords'] . '</label>';
		echo '<input name="keywords[]" id="keywords' . $row[0] . '" size="65" maxlength="75" type="text" value="' . cOut($row[1]) . '" />';
		echo '</div>';
		echo '<div class="frmOpt">';
		echo '<label for="description' . $row[0] . '">' . $hc_lang_settings['Description'] . '</label>';
		echo '<input name="descriptions[]" id="description' . $row[0] . '" size="80" maxlength="160" type="text" value="' . cOut($row[2]) . '" />';
		echo '</div>';
		echo '</fieldset><br />';
		++$cnt;
	}//end while?>
