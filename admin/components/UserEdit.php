...
<?php 
	if (isset($_GET['msg'])){
		switch ($_GET['msg']){
			...
			case "3" :
				feedback(2, "Email Address Already Exists. User Not Updated");
				break;
	...
	appInstructions(0, $where . " Newsletter Recipient", "Use the form below to add newsletter recipient to the system.");
...
<?
	...
	<tr>
		...
		<td class="eventMain">
			<?php
				if($addedby > 0){
					$result = doQuery("SELECT * FROM " . HC_TblPrefix . "admin WHERE PkID = '" . $addedby . "'");
					echo "<a class=\"main\" href=\"mailto:" . mysql_result($result,0,3) . "\">" . mysql_result($result,0,1) . " " . mysql_result($result,0,2) . "</a>";
				} else {
				?>
					Public Sign-Up
				<?
