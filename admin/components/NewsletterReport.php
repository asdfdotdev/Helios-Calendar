 ...
 */
	...
	if(hasRows($result)){
		...
		<fieldset>
			...
			<label>&nbsp;</label>
			<span class="output">
				<b>'.number_format(Amysqlresult($result,0,13),0,'.',',').'</b> '.$hc_lang_news['SentStats'].'
			</span>
			<label>&nbsp;</label>
			<span class="output">
				<b>'.number_format(Amysqlresult($result,0,14),0,'.',',').'</b> '.$hc_lang_news['ViewStats'].'
			</span>
			<label>&nbsp;</label>
			<span class="output">
				<b>'.number_format(Amysqlresult($result,0,15),0,'.',',').'</b> '.$hc_lang_news['ArchStats'].'
			</span>
		</fieldset>
		<fieldset>
			...
			<span class="output">
				<a href="mailto:'.Amysqlresult($result,0,12).'">'.trim(Amysqlresult($result,0,10).' '.Amysqlresult($result,0,11)).'</a>
			</span>
			<label>'.$hc_lang_news['MailSent'].'</label>
			<span class="output">
				'.stampToDate(Amysqlresult($result,0,5), $hc_cfg[24]).'
			</span>
			<label>'.$hc_lang_news['MailSubject'].'</label>
			<span class="output">
				'.cOut(Amysqlresult($result,0,1)).'
			</span>
			<label>'.$hc_lang_news['Dates'].'</label>
			<span class="output">
				'.stampToDate(Amysqlresult($result,0,2), $hc_cfg[24]).' - '.stampToDate(Amysqlresult($result,0,3), $hc_cfg[24]).'
			</span>
			<label>'.$hc_lang_news['Template'].'</label>
			<span class="output">
				'.((Amysqlresult($result,0,9) != '') ? Amysqlresult($result,0,9) : $hc_lang_news['NoTemplateB']).'
			</span>
			<label>' . $hc_lang_news['ArchStatus'] . '</label>
			<span class="output">
				'.((Amysqlresult($result,0,7) == 1) ? $hc_lang_news['ArchStatus1'] : $hc_lang_news['ArchStatus0']).'
			</span>
		</fieldset>';
		if(Amysqlresult($result,0,8) != ''){
			echo '<fieldset><legend>' . $hc_lang_news['Message'] . '</legend>' . Amysqlresult($result,0,8) . '</fieldset><br />';
		}
	} else {
