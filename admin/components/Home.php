...
*/
	...
	<fieldset>
		...
		<table cellpadding="4" cellspacing="0" border="0" width="100%">
			...
			<tr>
				<?php
					$result = doQuery("SELECT COUNT(en.NetworkID), NetworkID FROM " . HC_TblPrefix . "eventnetwork en LEFT JOIN " . HC_TblPrefix . "events e ON (e.PkID = en.EventID) WHERE e.StartDate >= '" . $curDate . "' AND en.NetworkType = 3 AND en.IsActive = 1 GROUP BY en.NetworkType");?>
				<td class="eventMain">
				<?php
					echo $hc_lang_home['AdminTweets'];
					echo (hasRows($result) && mysql_result($result,0,0) != '') ? ' (<a href="http://twitter.com/' . substr(mysql_result($result,0,1),0,strpos(mysql_result($result,0,1),'/')) . '" target="_blank" class="eventMain">' . $hc_lang_home['ViewTweetsA'] . '</a>)</td>' : '';
				?>
				<td class="eventMain" align="right">
				<?php
