 ...
 */
	...
	while($i < $stop){
		...
		if(isset($_POST['doBitly']) && $eventIDs[$i] > 0){
			$shortLink = CalRoot . "/index.php?com=detail&eID=" . $eventIDs[$i];
			$eID = $eventIDs[$i];
			require('../../events/includes/api/bitly/ShortenURL.php');
		}//end if
		++$i;
	}//end while
	...
	if($sendmsg > 0 && $eventStatus != 2){
		$subject = ($eventStatus == 2) ? CalName . ' ' . $hc_lang_event['EmailSubjectD'] : CalName . ' ' . $hc_lang_event['EmailSubjectA'];
		$message = '<p>' . nl2br($message) . '</p>';

		if($eventStatus == 1){
			$message .= ($edittype == 1) ?
				'<p>' . $hc_lang_event['EmailLinkEvent'] . ' <a href="' . CalRoot . '/index.php?eID=' . $editthis . '">' . CalRoot . '/index.php?eID=' . $editthis . '</a>' :
				'<p>' . $hc_lang_event['EmailLinkSeries'] . ' <a href="' . CalRoot . '/index.php?com=serieslist&sID=' . $editthis . '">' . CalRoot . '/index.php?com=serieslist&sID=' . $editthis . '</a>';
			$message .= '</p>';
		}//end if

		reMail($subname, $subemail, $subject, $message, $hc_cfg79, $hc_cfg78);
	}//end if
