...
*/
	...
	<?
		...
		$eventStatus = mysql_result($result,0,17);
		$eventBillboard = mysql_result($result,0,18);
		$eventTitle = mysql_result($result,0,1);
		$eventDesc = mysql_result($result,0,8);
		...
		$contactName = mysql_result($result,0,13);
		$contactEmail = mysql_result($result,0,14);
		$contactPhone = mysql_result($result,0,15);
		$contactURL = mysql_result($result,0,24);
		$allowRegistration = mysql_result($result,0,25);
		$maxRegistration = mysql_result($result,0,26);
	...
	?>
	This event has been viewed <b><?echo mysql_result($result,0,28);?></b> times.<br><br>
	...
	?>
			...
			<td class="eventMain">
				...
				<?php
					$query = "	SELECT " . HC_TblPrefix . "categories.*, 1 as IsSelected
								...
								WHERE " . HC_TblPrefix . "categories.IsActive = 1 AND " . HC_TblPrefix . "eventcategories.EventID = " . $eID . "
								UNION
								SELECT " . HC_TblPrefix . "categories.*, 0 as IsSelected
								FROM " . HC_TblPrefix . "categories
									LEFT JOIN " . HC_TblPrefix . "eventcategories ON (" . HC_TblPrefix . "categories.PkID = " . HC_TblPrefix . "eventcategories.CategoryID)
								WHERE " . HC_TblPrefix . "categories.IsActive = 1 AND " . HC_TblPrefix . "eventcategories.EventID != " . $eID . " OR " . HC_TblPrefix . "eventcategories.EventID is null
		...
		<tr>
			...
			<?	if(isset($eventDesc)){
				...
				<?makeTinyMCE("eventDescription", "100%", "advanced", $passContent);?>
			</td>
		</tr>
		<tr><td colspan="2"><img src="<?echo CalAdminRoot;?>/images/spacer.gif" width="1" height="7" alt="" border="0"></td></tr>
		<tr>
					...
					<tr>
						...
						<td>
							<?php
								if($contactURL != "http://"){
							?>
								&nbsp;<a href="<?echo $contactURL?>" class="main" target="_blank"><img src="<?echo CalAdminRoot;?>/images/iconWebsite.gif" width="20" height="20" alt="" border="0"></a>
							<?php
								} else {
							?>
								&nbsp;
