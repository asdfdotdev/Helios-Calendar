 ...
 */
	...
	if(isset($_GET['eID']) && is_numeric($_GET['eID'])){
		...
		$hc_Side[] = array(CalAdminRoot . '/index.php?com=eventadd&amp;eID=' . cIn($eID),'iconRecycle.png',$hc_lang_event['LinkRecycle'],0);
	} elseif(isset($_GET['sID'])){
		$editString = "0";
		$eventID = cIn($_GET['sID']);
		$resultS = doQuery("SELECT PkID FROM " . HC_TblPrefix . "events WHERE SeriesID = '" . $eventID . "'");
	...
	if(!hasRows($result)){
		echo '<div style="padding:10px 0px 150px 0px;">' . $hc_lang_event['EditWarning'] . '</div>';
	} else {
		...
		if(mysql_result($result,0,19)){
			if(isset($_GET['sID'])){
				$hc_Side[] = array(CalAdminRoot . '/index.php?com=searchresults&srsID=' . mysql_result($result,0,19),'iconViewSeries.png',$hc_lang_event['LinkSeriesView'],0);
			} else {
				$hc_Side[] = array(CalAdminRoot . '/index.php?com=eventedit&sID=' . mysql_result($result,0,19),'iconEditGroup.png',$hc_lang_event['LinkSeriesEdit'],0);
			}//end if
		}//end if
...
<?php 	}//end if	?>
	...
	function sendReg(eID){
		if(confirm('<?php echo $hc_lang_event['Valid50'] . "\\n\\n          " . $hc_lang_event['Valid51'] . "\\n          " . $hc_lang_event['Valid52'];?>')){
			document.getElementById('eventSendRoster').disabled = true;
			document.getElementById('eventSendRoster').value = '<?php echo $hc_lang_core['Sending'];?>';
...
<?php
	if(isset($editString)){
		...
		if(hasRows($resultD)){
			...
			echo '<br /><br />';
			echo '<label for="makeseries"><input ';
			echo (isset($_GET['sID'])) ? 'checked="checked" ' : '';
			echo 'type="checkbox" name="makeseries" id="makeseries" class="noBorderIE" />' . $hc_lang_event['GroupCombine'] . '</label>';
			echo '<br />';
		}//end if
	}//end if	?>
	<fieldset>
...
<?php 	if($allowRegistration == 1){	?>
	...
	<?php 	$result = doQuery("SELECT COUNT(*) FROM " . HC_TblPrefix . "registrants WHERE EventID = '" . $eID . "'");
		...
		<label>&nbsp;</label>
		<input style="width: 120px;" name="eventRegistrants" id="eventRegistrants" type="button" value="<?php if(isset($_GET['r'])){echo $hc_lang_event['RegButton1a'];}else{echo $hc_lang_event['RegButton1b'];}?>" onclick="togRegistrants();" class="button" />&nbsp;&nbsp;&nbsp;<input style="width: 160px;" name="eventSendRoster" id="eventSendRoster" type="button" value="<?php echo $hc_lang_event['RegButton2'];?>" onclick="sendReg(<?php echo $eID;?>);" class="button" />&nbsp;&nbsp;&nbsp;<input style="width: 120px;" name="addRegistrant" id="addRegistrant" type="button" value="<?php echo $hc_lang_event['RegButton3'];?>" onclick="window.location.href='<?php echo CalAdminRoot;?>/index.php?com=eventregister&amp;eID=<?php echo $eID;?>';" class="button" />
		<br /><br />
	...
	<?php 	$result = doQuery("SELECT * FROM " . HC_TblPrefix . "registrants WHERE EventID = " . cIn($eID) . " ORDER BY RegisteredAt, GroupID, PkID");
			if(hasRows($result)){
				...
				while($row = mysql_fetch_row($result)){
					...
					echo ($cnt % 2 == 0) ? '<td class="main">' : '<td class="tblListHL">';
					echo ($row[11] != '') ? stampToDate(cOut($row[11]), $hc_cfg24 . ' ' . $hc_cfg23) : 'N/A';
					echo '</td>';
	...
	<?php
		...
		if(count($tweets) > 0){
			...
			foreach($tweets as $val){
				echo ' <a href="http://twitter.com/' . $hc_cfg63 . '/status/' . $val . '" class="eventMain" target="_blank">' . $val . '</a>';
			}//end for
			echo '<br /><br />';
		}//end if
