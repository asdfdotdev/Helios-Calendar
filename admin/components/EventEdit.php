 ...
 */
	...
	$result = doQuery("SELECT e.*, l.PkID, l.Name, l.Address, l.Address2, l.City, l.State, l.Zip, l.Country, COUNT(r.EventID) as RegCnt
					FROM " . HC_TblPrefix . "events e
						LEFT JOIN " . HC_TblPrefix . "locations l ON (e.LocID = l.PkID)
						LEFT JOIN " . HC_TblPrefix . "registrants r ON (e.PkID = r.EventID AND r.EventID = '". $eID ."')
					WHERE e.PkID = '" . $eID . "' AND e.IsActive = 1");
	if($eID < 1 || mysql_result($result,0,0) < 1){
		...
		$regUsed = mysql_result($result,0,47);
		...
		<fieldset>
			...
			foreach($dateString as $val){
				echo ($cnt % 8 == 0) ? stampToDate($val, $hc_cfg[24]).'<br />' : stampToDate($val, $hc_cfg[24]).', ';
				++$cnt;
			}
