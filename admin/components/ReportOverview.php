...
*/
	if(isset($_GET['print'])){
		...
		$hc_popDateFormat = cOut(mysql_result($result,0,0));
	...
	<div style="float:left;width:350px;">
		...
		<b><?php echo $hc_lang_reports['GeneratedBy'] . "</b> Helios Calendar " . HC_Version;?>
	</div>
...
<?php
		...
		include($hc_langPath . $_SESSION['LangSet'] . '/admin/reports.php');?>
		<div align="right"><img src="<?php echo CalAdminRoot;?>/images/icons/iconPrint.png" width="15" height="15" alt="" border="0" style="vertical-align:middle;" /> <a href="<?php echo CalAdminRoot;?>/components/ReportOverview.php?print=1" class="main" target="_blank"><?php echo $hc_lang_reports['Print'];?></a></div>
<?php
	}//end if	?>
	...
	<?php 	$now = date("Y-m-d");
		...
		<div class="overviewCountHL">
	<?php 	$result = doQuery("SELECT COUNT(*) FROM " . HC_TblPrefix . "users WHERE IsRegistered = 1");
			if(hasRows($result)){
				echo mysql_result($result,0,0);
			} else {
				echo "N/A";
		...
		<div class="overviewCount">
	<?php 	$result = doQuery("SELECT COUNT(*) FROM " . HC_TblPrefix . "users");
			if(hasRows($result)){
				echo mysql_result($result,0,0);
			} else {
				echo "N/A";
...
<?php 	$result = doQuery("	SELECT SUM(Views) as Views,
							...
							FROM " . HC_TblPrefix . "events
							WHERE IsActive = 1 AND StartDate >= '" . date("Y-m-d") . "'");
		...
		<div class="overviewDetails" style="padding-top:10px;line-height:11px;vertical-align:bottom;"><b>&nbsp;<br /><?php echo $hc_lang_reports['TotalStats'];?></b></div>
		...
		<div class="overviewAverage"><?php echo $aViews !=0 ? number_format($aViews / $pastEvents, 2, '.', ',') : "0";?></div>
		<div class="overviewTotal"><?php echo $aViews != 0 ? $aViews : "0";?></div>
		<div class="overviewDetailsHL"><?php echo $hc_lang_reports['MViews'];?></div>
		<div class="overviewAverageHL"><?php echo $aMViews != 0 ? number_format($aMViews / $pastEvents, 2, '.', ',') : "0";?></div>
		...
		<div class="overviewAverage"><?php echo $aDirections !=0 ? number_format($aDirections / $pastEvents, 2, '.', ',') : "0";?></div>
		...
		<div class="overviewAverageHL"><?php echo $aDownloads != 0 ? number_format($aDownloads / $pastEvents, 2, '.', ',') : "0";?></div>
		...
		<div class="overviewAverage"><?php echo $aEmail !=0 ? number_format($aEmail / $pastEvents, 2, '.', ',') : "0";?></div>
		<div class="overviewTotal"><?php echo $aEmail != 0 ? $aEmail : "0";?></div>
		<div class="overviewDetailsHL"><?php echo $hc_lang_reports['URLClicks'];?></div>
		<div class="overviewAverageHL"><?php echo $aURL != 0 ? number_format($aURL / $pastEvents, 2, '.', ',') : "0";?></div>
...
<?php 	if(isset($_GET['print'])){	?>
			<div style="clear:both;width:650px;padding: 10px 0px 0px 0px;">
				<span class="main"><b><?php echo $hc_lang_reports['Generated'];?> Helios Calendar <?php echo HC_Version;?> -- www.HeliosCalendar.com</b></span>
...
<?php 		$cnt = 0;
			while($row = mysql_fetch_row($result)){	?>
				<div class="overviewDetailS<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo $row[0];?></div>
...
<?php 	}//end if
		...
		if(hasRows($result)){	?>
			<div class="overviewDetailS" style="padding-top:10px;"><b><?php echo $hc_lang_reports['MostEmail'];?>Most Email to Friend</b></div>
			<div class="overviewDate" style="padding-top:10px;"><b><?php echo $hc_lang_reports['Date'];?></b></div>
			<div class="overviewTotal" style="padding-top:10px;"><b><?php echo $hc_lang_reports['Count'];?></b></div>
<?php 		$cnt = 0;
			while($row = mysql_fetch_row($result)){	?>
...
<?php 		$cnt = 0;
			while($row = mysql_fetch_row($result)){	?>
				<div class="overviewDetailS2<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo $row[0];?></div>
				<div class="overviewDate<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo stampToDate($row[1], $hc_popDateFormat);?></div>
				<div class="overviewDate<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo stampToDate($row[2], $hc_popDateFormat);?></div>
<?php 		$cnt++;
			}//end while
		} else {	?>
...
<?php 		$cnt = 0;
			while($row = mysql_fetch_row($result)){	?>
				<div class="overviewName<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo $row[0] . " " . $row[1];?></div>
				<div class="overviewEmail<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo $row[2];?></div>
				<div class="overviewDate<?php if($cnt % 2 == 1){echo "HL";}?>"><?php echo stampToDate($row[3], $hc_popDateFormat);?></div>
<?php 		$cnt++;
			}//end while
		} else {	?>
