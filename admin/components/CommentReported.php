		...
		while($row = mysql_fetch_row($result)){
			...
			if(hasRows($resultR)){
				...
				while($row = mysql_fetch_row($resultR)){
					...
					echo '<div class="theReportLabel">' . $hc_lang_comment['By'] . '</div> ' . $row[4] . '<br />';
					echo ($row[3] > 0) ? '<div class="theReportLabel">' . $hc_lang_comment['OpenID'] . '</div> ' . $row[10] . '&nbsp;&nbsp;<a href="' . CalAdminRoot . '/index.php?com=oidedit&amp;uID=' . $row[3] . '" class="main"><img src="' . CalAdminRoot . '/images/icons/iconUserEdit.png" width="16" height="16" alt="" border="0" style="vertical-align:middle;" /></a><br />' : '';
					echo '<div class="theReportLabel">' . $hc_lang_comment['From'] . '</div> ' . $row[7];
