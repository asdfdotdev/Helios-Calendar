...
?>
<?php
	appInstructions(0, "Event Category Administration", "Use the form below to add, edit &amp; delete event categories.<br>The number of events assigned to each category is available beside the category name.");
?>
<br>
<table cellspacing="0" cellpadding="0" border="0" width="100%">
	<tr>
		...
		<td width="200" class="eventMain" valign="top">
			<?php
				//$result = doQuery("SELECT * FROM " . HC_TblPrefix . "categories WHERE IsActive = 1 ORDER BY CategoryName");
				
				$result = doQuery("	SELECT " . HC_TblPrefix . "categories.PkID as CategoryID, " . HC_TblPrefix . "categories.Categoryname, 0 as EventCount
									FROM " . HC_TblPrefix . "categories
									WHERE " . HC_TblPrefix . "categories.IsActive = 1
									
									UNION
									
									SELECT " . HC_TblPrefix . "eventcategories.CategoryID, " . HC_TblPrefix . "categories.CategoryName, count(" . HC_TblPrefix . "eventcategories.CategoryID) as EventCount 
									FROM hc_eventcategories 
										LEFT JOIN " . HC_TblPrefix . "categories ON (" . HC_TblPrefix . "eventcategories.CategoryID = " . HC_TblPrefix . "categories.PkID) 
										LEFT JOIN " . HC_TblPrefix . "events ON (" . HC_TblPrefix . "eventcategories.EventID = " . HC_TblPrefix . "events.PkID) 
									WHERE " . HC_TblPrefix . "events.startdate >= NOW() AND " . HC_TblPrefix . "events.IsApproved = 1 AND " . HC_TblPrefix . "events.IsActive = 1 AND " . HC_TblPrefix . "categories.IsActive = 1 
									GROUP BY " . HC_TblPrefix . "eventcategories.CategoryID 
									ORDER BY CategoryName, EventCount DESC");
				...
				if(hasRows($result)){
					...
					<?php
						...
						?>
							...
							<tr>
								<td class="eventMain" <?if($cnt % 2 == 1){echo "bgcolor=\"#EEEEEE\"";}//end if?>>&nbsp;<?if($cID == $row[0]){echo "<b>" . $row[1] . "</b>";} else {echo $row[1];}//end if?> (<?echo $row[2];?>)</td>
								<td class="eventMain" <?if($cnt % 2 == 1){echo "bgcolor=\"#EEEEEE\"";}//end if?> align="right"><a href="<?echo CalAdminRoot;?>/index.php?com=eventcategorymanage&cID=<?echo $row[0];?>" class="main" title="Edit Category"><img src="<?echo CalAdminRoot;?>/images/iconEdit.gif" width="15" height="15" alt="" border="0"></a>&nbsp;&nbsp;</td>
								<td class="eventMain" <?if($cnt % 2 == 1){echo "bgcolor=\"#EEEEEE\"";}//end if?>><a href="javascript:;" onClick="doDelete('<?echo $row[0];?>');return false;" class="main" title="Delete Category"><img src="<?echo CalAdminRoot;?>/images/iconDelete.gif" width="15" height="15" alt="" border="0"></a></td>
