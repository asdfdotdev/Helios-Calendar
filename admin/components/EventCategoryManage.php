		...
		<td width="200" class="eventMain" valign="top">
			<?php
				$result = doQuery("	SELECT " . HC_TblPrefix . "categories.PkID as CategoryID, " . HC_TblPrefix . "categories.Categoryname, 0 as EventCount
									...
									FROM " . HC_TblPrefix . "eventcategories 
				...
				if(hasRows($result)){
					...
					<?php
						...
						?>
							...
							<tr>
								<td class="eventMain" <?if($cnt % 2 == 1){echo "bgcolor=\"#EEEEEE\"";}//end if?>>&nbsp;<?if($cID == $row[0]){echo "<b>" . cOut($row[1]) . "</b>";} else {echo cOut($row[1]);}//end if?> (<?echo cOut($row[2]);?>)</td>
