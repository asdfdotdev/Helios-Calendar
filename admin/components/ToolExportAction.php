 ...
 */
	...
	$expVars = array(
		...
		25 => array('tag'=>'[category_unique]', 'field'=>'59'),
	...
	if(hasRows($result)){
		...
		$query .= ($groupBy >= 2) ? 'MIN(e.StartDate), MAX(e.StartDate)' : 'NULL, NULL';
		...
		while($i <= $stop){
			...
			switch($expVars[$i]['tag']){
				...
				case '[event_time_end]':
					...
					break;
				case '[loc_name]':
					$replace = ($row[35] == 0) ? $row[2] : $row[40];
					break;
				case '[loc_address]':
					$replace = ($row[35] == 0) ? $row[3] : $row[41];
					break;
				case '[loc_address2]':
					$replace = ($row[35] == 0) ? $row[4] : $row[42];
					break;
				case '[loc_city]':
					$replace = ($row[35] == 0) ? $row[5] : $row[43];
					break;
				case '[loc_region]':
					$replace = ($row[35] == 0) ? $row[6] : $row[44];
					break;
				case '[loc_postal]':
					$replace = ($row[35] == 0) ? $row[7] : $row[46];
					break;
				case '[loc_country]':
					$replace = ($row[35] == 0) ? $row[37] : $row[45];
					break;
				case '[loc_url]':
				case '[contact_url]':
				...
				case '[date_series]':
					if($row[60] != '')
						$replace = ($row[60] != $row[61] && $row[61] != '') ?  stampToDate($row[60],$hc_cfg[24]) . ' - ' . stampToDate($row[61],$hc_cfg[24]) : stampToDate($row[60],$hc_cfg[24]);
					break;
				case '[event_date]':
					if($row[$expVars[$i]['field']] != ''){
						switch($dateFormat){
