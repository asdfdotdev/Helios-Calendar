 ...
 */
	...
	if(hasRows($result)){
		...
		$footer = hc_mysql_result($result,0,4);
		$ext = hc_mysql_result($result,0,5);
		$groupBy = hc_mysql_result($result,0,7);
		$sortBy = hc_mysql_result($result,0,8);
		$cleanUp = explode("\n",hc_mysql_result($result,0,9));
		$dateFormat = hc_mysql_result($result,0,10);
		$curDate = $curCategory = '';
