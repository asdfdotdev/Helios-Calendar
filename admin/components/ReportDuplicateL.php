 ...
 */
	...
	$result = doQuery("SELECT l1.PkID as `First`, l1.Name, l1.Address, l1.Address2, l1.City, l1.State, l1.Zip, l1.Country,
							(SELECT COUNT(*) FROM hc_events e WHERE e.IsActive = 1 AND e.LocID = l1.PkID) as Events,
						l2.PkID as `Duplicate`, l2.Name, l2.Address, l2.Address2, l2.City, l2.State, l2.Zip, l2.Country,
							(SELECT COUNT(*) FROM hc_events e WHERE e.IsActive = 1 AND e.LocID = l2.PkID) as DupEvents
	...
	if(hasRows($result)){
		...
		while($row = mysql_fetch_row($result)){
			...
			<li class="row uline header">
				...
				<div class="tools" style="width:15%;">
					...
					<a href="'.AdminRoot.'/index.php?com=locsearch&l='.((!isset($_GET['n'])) ? cOut($row[1]) : '\''.cOut($row[2]).'\'').'"><img src="'.AdminRoot.'/img/icons/merge.png" width="16" height="16" alt="" /></a>
				</div>
			</li>';
			}
		...
		function doDelete(dID){
			if(confirm("'.$hc_lang_reports['Valid13'].'\\n\\n          '.$hc_lang_reports['Valid14'].'\\n          '.$hc_lang_reports['Valid15'].'"))
				document.location.href = "'.AdminRoot.'/components/LocationEditAction.php?dpID=1&dID=" + dID;
		}
		//-->
		</script>';
	} else {
