...
<?	if (isset($_GET['msg'])){
	...
	<?	$firstname = cOut(mysql_result($result,0,1));
		...
		while($row = mysql_fetch_row($result)){	?>
			<div class="userListName<?if($cnt % 2 == 1){echo "HL";}?>"><?echo cOut($row[1]) . " " . cOut($row[2]);?></div>
			<div class="userListEmail<?if($cnt % 2 == 1){echo "HL";}?>"><?echo cOut($row[3]);?></div>
			<div class="userListTools<?if($cnt % 2 == 1){echo "HL";}?>"><a href="<?echo CalAdminRoot;?>/?com=useredit&amp;uID=<?echo cOut($row[0]);?>" class="main"><img src="<?echo CalAdminRoot;?>/images/icons/iconEdit.gif" width="15" height="15" alt="" border="0" align="middle" /></a>&nbsp;<a href="javascript:;" onclick="javascript:doDelete('<?echo cOut($row[0]);?>'); return false;" class="main"><img src="<?echo CalAdminRoot;?>/images/icons/iconDelete.gif" width="15" height="15" alt="" border="0" align="middle" /></a></div>
		<?	$cnt++;
		}//end while
	} else {	?>
		<br />
