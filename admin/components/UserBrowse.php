 ...
 */
	...
	$result = doQuery("SELECT PkID, NetworkType, NetworkName, NetworkID, Email, SignIns, LastSignIn, LastIP, Level, IsBanned,
						(SELECT COUNT(PkID) FROM hc_events e WHERE e.OwnerID = u.PkID AND e.IsActive = 1) as Events
	...
	if(hasRows($result)){
		...
		function doDelete(dID,b){
			if(confirm("'.$hc_lang_user['Valid01'].'\\n\\n          '.$hc_lang_user['Valid02'].'\\n          '.$hc_lang_user['Valid03'].'"))
				document.location.href = "'.AdminRoot.'/components/UserEditAction.php?dID=" + dID + "&b=" + b;
		}
		function doBan(bID,b){
			...
			if(confirm(ban_msg))
				document.location.href = "'.AdminRoot.'/components/UserEditAction.php?bID=" + bID + "&b=" + b;
		}
		//-->
		</script>';
	} else {
