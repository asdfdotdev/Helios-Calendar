...
*/
	...
	doQuery("DELETE " . HC_TblPrefix . "eventnetwork en FROM " . HC_TblPrefix . "eventnetwork en LEFT JOIN " . HC_TblPrefix . "events e ON (e.PkID = en.EventID) WHERE e.PkID IS NULL OR e.IsActive = 0 OR e.IsApproved = 0");
	doQuery("DELETE FROM " . HC_TblPrefix . "locations WHERE IsActive = 0");
	doQuery("DELETE " . HC_TblPrefix . "locationnetwork ln FROM " . HC_TblPrefix . "locationnetwork ln LEFT JOIN " . HC_TblPrefix . "locations l ON (l.PkID = ln.LocationID) WHERE l.PkID IS NULL OR l.IsActive = 0");
	...
	doQuery("DELETE " . HC_TblPrefix . "recomndslog rl FROM " . HC_TblPrefix . "recomndslog rl LEFT JOIN " . HC_TblPrefix . "oidusers oid ON (oid.PkID = rl.OIDUser) WHERE oid.PkID is NULL OR oid.IsActive != 1");
