	...
	if(!hasRows($result)){
		...
		<ul class="data">
			...
			<li class="row'.$hl.'">
				...
				<div style="width:20%;">'.(($row[5] != '') ? cOut($row[5]) : '&nbsp;').'</div>
		...
		function doDelete(dID){
			if(confirm("'.$hc_lang_news['Valid29'] . '\\n\\n          ' . $hc_lang_news['Valid30'] . '\\n          ' . $hc_lang_news['Valid31'].'"))
				document.location.href = "'.AdminRoot . '/components/MailCreateAction.php?dID=" + dID;
		}
		//-->
		</script>';
		} else {
		...
		if(hasRows($result)){
			...
			echo '
			<form name="frmNewsletter" id="frmNewsletter" method="post" action="' . AdminRoot . '/components/MailQueueAction.php" onsubmit="return chkFrm();">
