 ...
 */
	...
	$result = doQuery("SELECT a.PkID, a.FirstName, a.LastName, a.Email, a.LoginCnt, a.LastLogin, a.PAge,
					ap.EventEdit, ap.EventPending, ap.EventCategory, ap.UserEdit, ap.AdminEdit, ap.Newsletter, ap.Settings, ap.Tools, ap.Reports, ap.Locations,
	...
	$editEvent = $eventPending = $eventCategory = $userEdit = $adminEdit = $newsletter = $settings = $tools = $reports = $locEdit = 0;
	$notices = array();
	...
	if(hasRows($result)){
		...
		$notices = array_filter(explode(',',mysql_result($result,0,17)),'is_numeric');
		$fails = mysql_result($result,0,18);
	...
	<fieldset>
		...
		<label>'.$hc_lang_admin['Password'].'</label>
		<span class="output">
			'.((($aID == 0)) ? $hc_lang_admin['CreatedBy']:'<a href="http://www.refreshmy.com/documentation/?title=Helios:_Change_Admin_Password" target="_blank">'.$hc_lang_admin['PwdChange'].'</a>').'
		</span>
	</fieldset>
	<fieldset>
