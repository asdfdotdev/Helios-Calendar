...
<?php
	if (isset($_GET['msg'])){
		switch ($_GET['msg']){
			...
			case "3" :
				feedback(2, "Email Address Already Exists.<br>Admin Not Updated");
				break;
	...
	appInstructions(0, $where . " Admin Account", "Use the form below to <b>" . $where . "</b> an admin account.");
?>
<br>
		...
		<td valign="top">
			<?php
				$result = doQuery("	SELECT * FROM " . HC_TblPrefix . "admin
									...
									WHERE " . HC_TblPrefix  . "admin.PkID = " . $uID);
				
				if(hasRows($result)){
					$firstname = mysql_result($result,0,1);
					$lastname = mysql_result($result,0,2);
					$email = mysql_result($result,0,3);
					$oldEmail = mysql_result($result,0,3);
					$password = mysql_result($result,0,4);
					$editEvent = mysql_result($result,0,11);
					$eventPending = mysql_result($result,0,12);
					$eventCategory = mysql_result($result,0,13);
					$userEdit = mysql_result($result,0,14);
					$adminEdit = mysql_result($result,0,15);
					$newsletter = mysql_result($result,0,16);
					$settings = mysql_result($result,0,17);
					$reports = mysql_result($result,0,18);
			...
			<table cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					...
					<td>
						<input size="20" maxlength="50" type="text" name="lastname" id="lastname" value="<?echo $lastname;?>" class="input">
					</td>
					<td>&nbsp;</td>
				</tr>
				<tr><td colspan="2"><img src="<?echo CalAdminRoot;?>/images/spacer.gif" width="1" height="4" alt="" border="0"></td></tr>
