...
<?	if (isset($_GET['msg'])){
	...
	<form name="frm" id="frm" method="post" action="<?echo CalAdminRoot . "/components/AdminEditAction.php";?>" onsubmit="return chkFrm();">
	...
	<?	if($_SESSION['AdminPkID'] != $uID){	?>
		...
		<div class="frmOpt">
			...
			<label for="toolsAllow" class="adminEditRadio"><input <?if($tools == 1){echo "checked=\"checked\"";}//end if?> type="radio" name="tools" id="toolsAllow" value="1" class="noBorderIE" />Allow</label>
			<label for="toolsLocked" class="adminEditRadio"><input <?if($tools == 0){echo "checked=\"checked\"";}//end if?> type="radio" name="tools" id="toolsLocked" value="0" class="noBorderIE" />Locked</label>
			&nbsp;<? appInstructionsIcon("Tools", "Allows Administrator Access to Calendar Tools."); ?>
		</div>
		<div class="frmOpt">
