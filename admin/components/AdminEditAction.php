	...
	if(!isset($_GET['dID'])){
		...
		if(hasRows($result)){
			...
			if(hasRows($result)){
				...
				$pwKey = md5(date("U"));
				doQuery("INSERT INTO " . HC_TblPrefix . "admin(FirstName, LastName, Passwrd, Email, SuperAdmin, IsActive, PCKey) VALUES('" . cIn($firstname) . "', '" . cIn($lastname) . "', NULL, '" . cIn($email) . "', 0, 1, '" . cIn($pwKey) . "')");
