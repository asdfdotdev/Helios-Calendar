...
*/
	...
	<?php 	}//end if	?>
		...
		<div class="frmOpt">
			...
			<?php echo $hc_lang_locations['Latitude'];?>&nbsp;<input name="lat" id="lat" type="text" value="<?php echo $lat;?>" size="12" maxlength="25" />&nbsp;&nbsp;<?php echo $hc_lang_locations['Longitude'];?>&nbsp;<input name="lon" id="lon" type="text" value="<?php echo $lon;?>" size="12" maxlength="25" />
		<?php
			if($lat != '' && $lon != ''){
				echo '<a href="' . $hc_cfg52 . '/maps?q=' . $lat . ',' . $lon . '" target="_blank" title="' . $hc_lang_locations['Preview'] . '"><img src="' . CalAdminRoot . '/images/icons/iconMap.png" width="16" height="16" alt="' . $hc_lang_locations['Preview'] . '" border="0" style="vertical-align:middle;" /></a>';
			}//end if?>
		</div>
		<div class="frmOpt">
			<label>&nbsp;</label>
			<label for="updateMap" class="category"><input name="updateMap" id="updateMap" type="checkbox" <?php if($hc_cfg26 == ''){echo 'disabled="disabled"';} if($lID == 0){echo 'checked="checked"';}?> class="noBorderIE" /><?php echo $hc_lang_locations['Update'];?></label>&nbsp;
		</div>
		<div class="frmOpt">
			<label>&nbsp;</label>
		...
		<?php
			makeTinyMCE('descript', '525px', 'advanced', $descript);?>
		</div>
	</fieldset>
	<br />
	<fieldset>
	...
	<?php
		...
		echo '<div class="frmOpt" style="border-top:solid 1px #CCCCCC;"><label for="doEventbrite" class="radioDistPub"><input name="doEventbrite" id="doEventbrite" type="checkbox" onclick="toggleMe(document.getElementById(\'eventbrite\'));"';
		...
		echo $hc_lang_locations['EventbriteNotice'];
		echo '</div>';?>
	</fieldset>
	<br />
