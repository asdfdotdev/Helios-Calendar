...
*/
	...
	$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings WHERE PkID IN (1,2,3,4,8,9,10,11,12,13,14,15,21,22,23,24,25,28,29,30,31,32,33,34,35,40,44,48,53,56,65,66,67,68) ORDER BY PkID");
...
<?php	$capDisplay = "";
		$noGD = (!function_exists('imagecreate')) ? true : false;
		$canCap = ($noGD == true || $capType == 0) ? 'disabled="disabled"' : '';

		echo $hc_lang_settings['reCAPTCHANotice'] . '<a href="http://recaptcha.net/terms.html" class="eventMain" target="_blank">' . $hc_lang_settings['TermsOfUse'] . '</a>.';
		...
		echo '</select> (<a href="http://recaptcha.net/" target="_blank" class="eventMain">' . $hc_lang_settings['reCAPTCHALink'] . '</a>)</div>';
