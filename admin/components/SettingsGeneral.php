 ...
 */
	...
	$result = doQuery("SELECT SettingValue FROM " . HC_TblPrefix . "settings
					WHERE PkID IN (1,2,3,4,8,9,10,11,12,13,14,15,21,22,23,24,26,28,29,30,31,32,33,34,35,40,44,48,
								53,54,65,66,67,68,70,71,72,73,74,75,76,77,78,79,80,81,82,87,88,89,90,91,92,93)
					ORDER BY PkID");
	...
	$mailMethod = cOut(mysql_result($result,35,0));
	$mailHost = cOut(mysql_result($result,36,0));
	$mailPort = cOut(mysql_result($result,37,0));
	$mailSecure = cOut(mysql_result($result,38,0));
	$mailUser = cOut(mysql_result($result,39,0));
	$mailPass = base64_decode(cOut(mysql_result($result,40,0)));
	$mailAuth = cOut(mysql_result($result,41,0));
	$mailAddress = cOut(mysql_result($result,42,0));
	$mailName = cOut(mysql_result($result,43,0));
	$loginTries = cOut(mysql_result($result,44,0));
	$batchSize = cOut(mysql_result($result,45,0));
	$batchDelay = cOut(mysql_result($result,46,0));
	$sitemap = cOut(mysql_result($result,47,0));
	$iCalMax = cOut(mysql_result($result,48,0));
	$iCalRef = cOut(mysql_result($result,49,0));
	$reCapStyle = cOut(mysql_result($result,50,0));
	$passStr = cOut(mysql_result($result,51,0));
	$mc_select = cOut(mysql_result($result,52,0));
	$mc_dow = cOut(mysql_result($result,53,0));
	...
	echo '
		...
		<select name="WYSIWYG" id="WYSIWYG">
			<option'.(($WYSIWYG == 1) ? ' selected="selected"':'').' value="1">'.$hc_lang_settings['WYSIWYGY'].'</option>
	...
	<fieldset>
		...
		<div id="phpMailer" style="'.(($mailMethod == 0) ? 'display:none;' : '').'">
			...
			<input'.$mActive.' name="mailPassChg" id="mailPassChg" type="password" size="10" maxlength="50" value="" />
			<label for="mailPassCon">'.$hc_lang_settings['MailPassCon'].'</label>
			<input'.$mActive.' name="mailPassCon" id="mailPassCon" type="password" size="10" maxlength="50" value="" />
		</div>
	...
	echo '
		...
		<label>'.$hc_lang_settings['HeliosLP'].'</label>
		<span class="output">
			<a href="https://www.refreshmy.com/members/" target="_blank">'.$hc_lang_settings['DownloadHere'].'</a>
		</span>
		<label>'.$hc_lang_settings['TinyMCELP'].'</label>
		<span class="output">
			<a href="http://www.tinymce.com/wiki.php/Language" target="_blank">'.$hc_lang_settings['DownloadHere'].'</a>
		</span>
	</fieldset>
	<fieldset>
	...
	<fieldset>
		<legend>'.$hc_lang_settings['BillPopRSS'].'</legend>
		...
		<input name="maxRSS" id="maxRSS" type="number" min="1" max="99" size="3" maxlength="2" required="required" value="'.$maxDisplay.'" />
		<label for="iCalMax">'.$hc_lang_settings['iCalMax'].'</label>
		<input name="iCalMax" id="iCalMax" type="number" min="1" max="99" size="4" maxlength="3" required="required" value="'.$iCalMax.'" />
	...
	<fieldset>
		...
		<p>
			'.$hc_lang_settings['LinksNotice'].' <a href="http://www.refreshmy.com/documentation/?title=Helios:_Weather/Map_Links" target="_blank">' . $hc_lang_settings['LinkDoc'] . '</a>
		</p>
	...
	<fieldset>
		...
		<textarea name="defaultApprove" id="defaultApprove" rows="15">'.$passApprove.'</textarea>
		<label for="defaultDecline">'.$hc_lang_settings['DecMessage'].'</label>
		<textarea name="defaultDecline" id="defaultDecline" rows="15">'.$passDecline.'</textarea>
	</fieldset>
	...
	function validate(){
		...
		if(document.getElementById("display").value != ""){
			...
			err +=validGreater(document.getElementById("display"),-1,"'.$hc_lang_settings['Valid13'].'\n");
		}
		...
		if(document.getElementById("mostPopular").value != ""){
			...
			err +=validGreater(document.getElementById("mostPopular"),-1,"'.$hc_lang_settings['Valid10'].'\n");
		}
		...
		if(document.getElementById("maxNew").value != ""){
			...
			err +=validGreater(document.getElementById("maxNew"),-1,"'.$hc_lang_settings['Valid53'].'\n");
		}
		...
		if(document.getElementById("maxRSS").value != ""){
			...
			err +=validGreater(document.getElementById("maxRSS"),-1,"'.$hc_lang_settings['Valid07'].'\n");
		}
	...
	function sendTestMsg(){
		if(confirm("'.$hc_lang_settings['Valid33'].'")){
			...
			if(err != ""){
				...
				window.open("'.AdminRoot.'/components/EmailTest.php?e=" + document.getElementById("mailTest").value,"hc_mailtest","location=1,status=1,scrollbars=1,width=800,height=300,left="+(screen.availWidth/2-400)+",top="+(screen.availHeight/2-300));
			}
		} else {
			return false;
		}
	...
	makeTinyMCE('defaultApprove,defaultDecline',0,0,0);
?>