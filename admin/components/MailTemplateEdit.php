 ...
 */
	...
	echo '
		...
		<textarea name="tempsource" id="tempsource" rows="30">'.$source.'</textarea>
	</fieldset>
	...
	makeTinyMCE('tempsource',1,$ovrEdit,1);
	} else {
		if(isset($_GET['msg'])){
			switch ($_GET['msg']){
				case "1" :
	...
	function doDelete(dID){
		if(confirm("'.$hc_lang_news['Valid20'].'\\n\\n          '.$hc_lang_news['Valid21'].'\\n          '.$hc_lang_news['Valid22'].'"))
			document.location.href = "'.AdminRoot.'/components/MailTemplateEditAction.php?dID=" + dID;
	}
