...
<?php 
	if (isset($_GET['msg'])){
		...
		if($step == 0){
			appInstructions(0, "Event Newsletter", "Step 1 of 2) Customize your newsletter settings.");
		?>
			<br>
			<div align="right"></div>
		...
		<?php
				...
				appInstructions(1, "Event Newsletter", "Step 2 of 2) Confirm settings and review newsletter event stats.<br>If you wish to send click '<i>Send Now</i>', or click '<i>Start Over</i>' to stop.");
		?>
		<br>
		<ul>
		<?php
			...
			<tr>
				...
				?>
					...
					<?
						while($row = mysql_fetch_row($result)){
						?>
							<option <?if($template == $row[0]){echo "SELECTED";}//end if?> value="<?echo $row[0];?>"><?echo $row[1];?></option>
						<?
						}//end while
					?>
					</select>
