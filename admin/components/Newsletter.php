...
*/
	...
	if($step == 0){
		...
		function chkFrm(){
			...
			if(compareDates('<?php echo strftime($hc_popDateFormat);?>', '<?php echo $hc_popDateValid;?>', startDate, '<?php echo $hc_popDateValid;?>') == 1){
				dirty = 1;
			...
			if(!isDate(document.frmNewsletter.startDate.value, '<?php echo $hc_popDateValid;?>')){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_news['Valid15'] . " " . strtolower($hc_popDateValid);?>';
			...
			if(!isDate(document.frmNewsletter.endDate.value, '<?php echo $hc_popDateValid;?>')){
				dirty = 1;
				warn = warn + '\n<?php echo $hc_lang_news['Valid17'] . " " . strtolower($hc_popDateValid);?>';
		...
		<fieldset>
			...
			<div class="frmOpt">
				...
				<input size="12" maxlength="10" type="text" name="startDate" id="startDate" value="<?php echo strftime($hc_popDateFormat,mktime($hourOffset,date("i"),date("s"),date("m"),date("d"),date("Y")));?>" />&nbsp;<a href="javascript:;" onclick="calx.select(document.frmNewsletter.startDate,'anchor1','<?php echo $hc_popDateValid;?>'); return false;" name="anchor1" id="anchor1"><img src="<?php echo CalAdminRoot;?>/images/icons/iconCalendar.png" width="16" height="16" border="0" alt="" /></a>
				&nbsp;&nbsp;to&nbsp;&nbsp;
				<input size="12" maxlength="10" type="text" name="endDate" id="endDate" value="<?php echo strftime($hc_popDateFormat,mktime($hourOffset,date("i"),date("s"),date("m"),date("d") + 7,date("Y")));?>" />&nbsp;<a href="javascript:;" onclick="calx.select(document.frmNewsletter.endDate,'anchor2','<?php echo $hc_popDateValid;?>'); return false;" name="anchor2" id="anchor2"><img src="<?php echo CalAdminRoot;?>/images/icons/iconCalendar.png" width="16" height="16" border="0" alt="" /></a>
			</div>
			<div class="frmOpt">
...
<?php 	}//end if
	...
	<?php 	$result = doQuery("	SELECT count(*), MIN(StartDate)
			...
			if(mysql_result($result,0,1) != ''){
				$sendOk = 1;
				echo "<br /><br /><b>" . $hc_lang_news['FirstUnsent'] . "</b> " . stampToDate(mysql_result($result,0,1), $hc_dateFormat);
			}//end if	?>
		<div style="width:400px;">
