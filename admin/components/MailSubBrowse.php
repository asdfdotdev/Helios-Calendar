 ...
 */
	...
	$hc_Side[] = array(AdminRoot . '/components/MailSubEditAction.php?dID=uc&a=1','user_delete.png',$hc_lang_news['DeleteNoConfirm'] . ' <b>' . $num . '</b>',0);
	...
	echo ($resOffset > $resDiff) ? '<a href="'.AdminRoot.'/index.php?com=submngt&p=0&a='.$resLimit.'">1</a>&nbsp;...&nbsp;' : '';
	...
	echo ($resOffset < ($pages - ($resDiff + 1))) ? '&nbsp;...&nbsp;<a href="'.AdminRoot.'/index.php?com=submngt&p='.($pages - 1).'&a='.$resLimit.'">'.$pages.'</a>' : '';
	echo '
			</span>
			<label>&nbsp;</label>
			<span class="frm_ctrls">
				<input name="filter" id="filter" type="text" size="30" maxlength="50" value="'.$term.'" />
				<input name="filter_go" id="filter_go" type="button" value="'.$hc_lang_news['FilterNews'].'" onclick="window.location.href=\''.AdminRoot.'/index.php?com=submngt&p=0&a='.$resLimit.'&s=\'+document.getElementById(\'filter\').value;" />
			</span>
	...
	if(hasRows($result)){
		...
		<ul class="data">
			<li class="row header uline">
				<div style="width:30%;">'.$hc_lang_news['Name'].'</div>
				<div style="width:35%;">'.$hc_lang_news['Emailb'].'</div>
				<div style="width:25%;">'.$hc_lang_news['Registered'].'</div>
		...
		while($row = mysql_fetch_row($result)){
			...
			echo '
			<li class="row'.$hl.'">
				<div style="width:30%;">'.cOut(trim($row[2].', '.$row[1])).'</div>
				<div class="txt" title="'.cOut($row[3]).'" style="width:35%;">'.cOut($row[3]).'&nbsp;</div>
		...
		function doDelete(dID){
			if(confirm("'.$hc_lang_news['Valid08'].'\\n\\n          '.$hc_lang_news['Valid09'].'\\n          '.$hc_lang_news['Valid10'].'"))
				document.location.href = "'.AdminRoot.'/components/MailSubEditAction.php?dID=" + dID;
		}
		//-->
		</script>';
	} else {
