...
<?php	
	...
	$result = doQuery("SELECT COUNT(PkID)
						FROM " . HC_TblPrefix . "events
						WHERE IsActive = 0 OR IsApproved = 0 OR StartDate = '0000-00-00'");
	...
	$result = doQuery("SELECT COUNT(*)
						FROM " . HC_TblPrefix . "eventcategories ec
							LEFT JOIN " . HC_TblPrefix . "events e ON (ec.EventID = e.PkID)
						WHERE e.PkID is NULL OR e.IsActive = 0 OR e.IsApproved = 0");
	...
	$result = doQuery("SELECT COUNT(PkID)
						FROM " . HC_TblPrefix . "commentsreportlog
						WHERE IsActive = 0");
	...
	if($pEvents > 0 || $pLoc > 0 || $pCats > 0 || $pEventCat > 0 || $oidUsers > 0 || $pComments > 0 || $pCommentLog > 0){
