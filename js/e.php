 ...
 */
	...
	if(!file_exists($cache)){
		...
		$query = "SELECT DISTINCT e.PkID, e.Title, e.StartDate, e.StartTime, e.TBD, e.IsBillboard, e.Views, e.PublishDate, GROUP_CONCAT(c.PkID ORDER BY c.PkID)
				...
				WHERE e.IsActive = 1 AND e.IsApproved = 1 AND c.IsActive = 1 AND e.StartDate >= '".SYSDATE." ".SYSTIME."' AND c.IsActive = 1 ";
		...
		switch($sID){
			...
			case 2:
				...
				break;
			case 3:
				$query .= " AND e.IsBillboard = 1 ".$gQuery." ORDER BY e.StartDate, e.TBD, e.Title LIMIT " . $hc_cfg[60];
				break;
		}
