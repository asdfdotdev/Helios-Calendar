...
*/
	...
	function php_check($version){
		return ($version != '' && $version >= '5.2.0') ? true : false;
	}
	function mysql_check($version){
		return ($version != '' && $version >= '5.1.0') ? true : false;
	}
	function progress(){
		global $sID;
		...
		<span'.(($sID == 3) ? ' class="active"' : '').'>License Code</span> &raquo;
		<span'.(($sID == 4) ? ' class="active"' : '').'>Account &amp; Database Setup</span> &raquo;
		<span'.(($sID == 0) ? ' class="active"' : '').'>Finished</span>' : '<b>Welcome to the Helios Calendar upgrade.</b>';
	}
	function get_step(){
		if(!isset($_SESSION['code_valid']) || $_SESSION['code_valid'] == false){
