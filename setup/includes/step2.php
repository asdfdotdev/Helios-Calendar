...
*/
	...
	if(!isset($_SESSION['license']) || $_SESSION['license'] == false){
		...
		$pass = 0;
		$cf_class = $cd_class = $up_class = $php_class = $mysql_class = $dbh_class = $dbn_class = $dbu_class = $dbp_class = $tbp_class = $cr_class = $ar_class = $cn_class = $rnd_class = $mys_class = 'error';
		...
		try {
			...
			if(mysql_check(mysql_result($result,0,0))){
				$mysql_class = 'ok';
				++$pass;}
		...
		<fieldset>
			...
			<p>
				...
				<ol>
					<li>The MySQL user (DB_USER) listed above must have Select/Insert/Update/Delete/Create/Alter privileges granted for the database (DB_NAME) listed above.</li>
