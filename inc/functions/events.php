 ...
 */
	...
	function event_browse($nav_function = 'event_browse_nav'){
		...
		while($row = mysql_fetch_row($result)){
			...
			echo '
			<li'.$hl.' itemscope itemtype="http://schema.org/Event"><time itemprop="startDate" datetime="'.$stamp.'">'.$time.'</time><a itemprop="url" href="'.CalRoot . '/index.php?eID='.$row[0].$location.'"><span itemprop="name">'.cOut($row[1]).'</span></a></li>';
			++$cnt;
		}
		echo '</ul>
		'.$myNav;
