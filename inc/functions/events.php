 ...
 */
	...
	function event_fetch(){
		...
		$result = doQuery("SELECT e.PkID, e.Title, e.Description, e.StartDate, e.StartTime, e.EndTime, e.TBD, e.ContactName, e.ContactEmail, e.ContactURL, e.ContactPhone, e.AllowRegister, 
							e.SpacesAvailable, e.LocID, e.SeriesID, e.Cost, e.LocationName, e.LocationAddress, e.LocationAddress2, e.LocationCity, e.LocationState, e.LocationZip, e.LocCountry, 
							e.ShortURL, e.LastMod, e.Image, l.Name, l.Address, l.Address2, l.City, l.State, l.Zip, l.Country, l.URL, l.Phone, l.Email, l.Lat, l.Lon,
	 ...
	 * @return void 
	 */	
	function event_browse_valid($nav_function = 'event_browse_nav'){
		...
		if($endDate < $hc_cfg['First']){
			go_home();
		} else {
			$myNav = call_user_func($nav_function,$startDate,$endDate,$window,$location);
			
			$resultEB = doQuery("SELECT DISTINCT e.PkID, e.Title, e.StartDate, e.StartTime, e.EndTime, e.TBD, e.Image
							FROM " . HC_TblPrefix . "events e
							...
							GROUP BY e.PkID, e.Title, e.StartDate, e.StartTime, e.EndTime, e.TBD, e.Image
	...
	function event_browse($show_images = 0){
		...
		while($row = mysql_fetch_row($resultEB)){
			...
			echo '
			<li'.$hl.' itemscope itemtype="http://schema.org/Event">
				<time itemprop="startDate" datetime="'.$stamp.'">'.$time.'</time><a itemprop="url" href="'.CalRoot . '/index.php?eID='.$row[0].$location.'"><span itemprop="name">'.cOut($row[1]).'</span></a>'.(($show_images == 1 && $row[6] != '') ? '
				<img src="'.$row[6].'" class="eimage_b" />':'').'
			</li>';
			++$cnt;
		}
		echo '
	...
	function event_ical_link($eID){
		...
		echo '
		<a href="http://192.168.2.114/helios/link/SaveEvent.php?eID='.$eID.'&cID=3" class="icon ical">'.$hc_lang_event['iCalendar'].'</a><br />';
	}
	/**
