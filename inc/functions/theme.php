 ...
 */
	...
	function cal_menu_items(){
		...
		$menu_items[] = array(CalRoot.'/index.php', $hc_lang_core['Events'], '');
		if($hc_cfg[45] == 1 && ($hc_cfg[42] != '' && $hc_cfg[43] != ''))
			$menu_items[] = array(CalRoot.'/index.php?com=location', $hc_lang_core['Locations'],'');
		if($hc_cfg[1] == 1)
			$menu_items[] = array(CalRoot.'/index.php?com=submit', $hc_lang_core['Submit'],'');
		$menu_items[] = array(CalRoot.'/index.php?com=search', $hc_lang_core['Search'], '');
		if($hc_cfg[54] == 1)
			$menu_items[] = array(CalRoot.'/index.php?com=newsletter', $hc_lang_core['Newsletter'],'');
		$menu_items[] = array(CalRoot.'/index.php?com=tools', $hc_lang_core['Tools'],'');
		if($hc_cfg[97] == 1)
			$menu_items[] = array(CalRoot.'/index.php?com=digest', $hc_lang_core['WhatsNew'],'');
		if(user_check_status()){
			$menu_items[] = array(CalRoot.'/index.php?com=acc', $hc_lang_core['Account'],' usr usr_ac');
			$menu_items[] = array(CalRoot.'/signout.php', $hc_lang_core['SignOut'],' usr usr_so');
		} elseif(($hc_cfg[113]+$hc_cfg[114]+$hc_cfg[115]) > 0 && !user_check_status()){
			$menu_items[] = array(CalRoot.'/index.php?com=signin', $hc_lang_core['SignIn'],' usr usr_si');
		}
