 ...
 */
	 ...
	 * Writes cache files.
	 * @since 2.0.0
	 * @param int $cID file to be created 0 = config.php, 1 = currently unused, 2 = locList(a).php, 3 = meta.php, 4 = selCity(a).php, 5 = selPostal(a).php (Default:0)
	 * @param int $a [optional] 0 = public calendar, 1 = admin console (Default:0)
	 * @return void
	 */
		...
		switch($cID){
			...
			case 3:
				if(!file_exists(HCPATH . '/cache/meta.php')){
					...
					$x = 1;
					$pairs = array(1 => 1,2 => 2,3 => 'submit',4 => 'search',5 => 'searchresult',6 => 'signup',7 => 'send',8 => 'rsvp',9 => 'tools',10 => 'rss',11 => 'newsletter',12 => 'archive',13 => 'filter');
			...
			default:
				if(!file_exists(HCPATH . '/cache/settings.php')){
					...
					if(hasRows($result)){
						...
						fwrite($fp, "00 => \"hc_" . sha1(md5(CalRoot) . HC_Rando) . "\",\n");
						fwrite($fp, "000 => \"helios_" . md5(CalRoot . HC_Rando) . "\",\n");
						fwrite($fp, "\"CatCols\" => \"3\",\n");
						...
						if(hasRows($resultE)){
							fwrite($fp, "\"First\" => \"" . strtotime(mysql_result($resultE,0,0)) . "\",\n");
	...
	/**
	 * Filter value for use in MySQL query to protect against SQL injection, wrapper for mysql_real_escape_string().
	 * @since 2.0.0
	 * @version 2.0.0
	 * @param string $value string to be filtered
	 * @param integer $filter [optional] 0 = Maintain Double Quotes, 1 = Replace Double Quotes w/Single Quotes (Default:1)
	 ...
	 * @since 2.0.0
	 * @version 2.0.0
	 * @param string $textName comma separated list of textarea ids to convert to TinyMCE editors.
	 ...
	 * @param integer $fullpage [optional - only available for admin style editors] 0 = do not include fullpage plug-in or doctype in editor contents, 1 = include fullpage plug-in and add doctype to editor contents (Default:0)
	 * @return void
	 */
	function makeTinyMCE($textName, $admin = 0, $override = 0, $fullpage = 0){
	...
	//<!--
	tinyMCE_GZ.init({
		plugins :  "imagemanager,spellchecker,advhr,advimage,advlink,advlist,autolink,autoresize,contextmenu'.(($admin == 1 && $fullpage == 1) ? ',fullpage':'').',fullscreen,inlinepopups,insertdatetime,layer,media,paste,preview,searchreplace,style,table,visualchars,wordcount,xhtmlxtras",
		themes : "advanced",languages : "en",disk_cache : true,debug : false});
	tinyMCE.init({
		...
		elements : "'.$textName.'",
		entity_encoding : "raw",
		plugins :  "imagemanager,spellchecker,advhr,advimage,advlink,advlist,autolink,autoresize,contextmenu,emotions'.(($admin == 1 && $fullpage == 1) ? ',fullpage':'').',fullscreen,inlinepopups,layer,media,paste,preview,searchreplace,style,tabfocus,table,visualchars,wordcount,xhtmlxtras",
		'.(($admin == 1) ? 'theme_advanced_buttons1 : "fontsizeselect,|,fontselect,|'.(($admin == 1 && $fullpage == 1) ? ',fullpage,|,':'').',bold,italic,underline,strikethrough,|,bullist,numlist,|,justifyleft,justifycenter,justifyright,|,outdent,indent,sup,sub,|,replace",
		...
		theme_advanced_buttons3 : "tablecontrols,visualaid,insertlayer,moveforward,movebackward,absolute,|,advhr,|,visualchars,code,|,fullscreen,preview,|,spellchecker,|,help",
	...
	function feedback($type, $fmessage){
		...
		echo '
		<div id="hc_dialog" class="feedback '.$fType.'"><img src="'.CalRoot.'/img/feedback/'.$fType.'.png" alt="" />&nbsp;'.$fmessage.'</div>';
	}
	/**
	 ...
	 * @return void
	 */
	function reMail($toName,$toAddress,$subj,$msg,$fromName = '',$fromAddress = '',$debug = false){
		global $hc_lang_core,$hc_cfg;
		...
		$mail->AddCustomHeader($mail->HeaderLine('X-Helios-Calendar-Report-Abuse','http://www.refreshmy.com/abuse/'));
		$mail->CreateHeader();
		$mail->IsHTML(true);
	 ...
	 * @since 2.0.0
	 * @version 2.0.0
	 * @param integer $admin [optional] manual override for admin console 1 = hide notice & force reCAPTCHA theme, 0 = output notice & use reCAPTCHA theme settings (Default:0)
	 * @return void
	 */
	function buildCaptcha($admin = 0){
		...
		if($hc_cfg[65] == 1){
			...
			echo ($admin == 0) ? '<p>'.$hc_lang_core['CapNotice'] : '';
			echo '
			<label class="blank">&nbsp;</label>
			<img src="'.CalRoot.'/inc/captcha.php" width="250" alt="" id="cap_img" />
			<br /><br />
