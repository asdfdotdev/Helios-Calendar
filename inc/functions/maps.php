 ...
 */
	 ...
	 * @param integer $type Embedded map type: 1 = Single Point Map (Pushpin at passed coordinates), 2 = Multiple Point Map (Pushpins from Venues Cache)
	 ...
	 * @return void
	 */
	function get_map_js($lat,$lon,$type,$pin_icon = '',$pin_action = 0,$pin_content = ''){
		global $hc_cfg;
	 ...
	 * @return void
	 */
	function gmap_venues_init($lat,$lon,$pin_icon){
	 ...
	 * @return void
	 */
	function olayers_venues_init($lat,$lon,$pin_icon){
	...
	function map_add_venues(locations){
		for(var i = 0; i < locations.length; i++) {
			if(locations[i] != undefined){
				...
				map_create_marker(lonLat, \'<div class="iw"><div class="iw_menu">'.map_infowindow_menu().'</div><b>\'+location[1]+\'</b><br />\'+map_set_iw(location)+\'</div>\');
			}
		}		
	}
	function map_create_marker(lonLat, popupContent) {
		var size = new OpenLayers.Size(32,37);
		var offset = new OpenLayers.Pixel(-(size.w/2), -size.h);
		var icon = new OpenLayers.Icon(\''.$pin_icon.'\',size,offset);
	...
	function map_set_iw(loc){
		...
		iw += (loc[9] != "") ? "<br />"+loc[9] : "";
		iw += "<br />";
		iw += (loc[13] == 1) ? "<br />'.$hc_lang_locations['Website'].' <a href=\"'.CalRoot.'/link/index.php?tID=4&oID="+loc[0]+"\" target=\"_blank\">'.$hc_lang_locations['ClickToVisit'].'</a>" : "";
		iw += (loc[14] != "") ? "<br />'.$hc_lang_locations['Phone'].' "+loc[14] : "";
		iw += (loc[2] != "" && loc[3] != "") ? "<br /><br />"+loc[2]+", "+loc[3] : "";
		iw += "<br />";
		iw += (loc[11] != "") ? "<br />'.$hc_lang_locations['EventCnt'].' " +loc[11] : "";
		iw += (loc[12] != "") ? "<br />'.$hc_lang_locations['Next'].' " +loc[12] : "";
		return iw;
	}';
	}
	/**
	...
	function map_infowindow_menu(){
		...
		return '<a target="_blank" href="'.CalRoot.'/?lID=\'+location[0]+\'" title="'.$hc_lang_locations['MnuCalendar'].'"><img src="'.CalRoot.'/img/icons/calendar.png" width="16" height="16" /></a>'
		.'<a target="_blank" href="'.CalRoot.'/index.php?com=location&lID=\'+location[0]+\'" title="'.$hc_lang_locations['MnuProfile'].'"><img src="'.CalRoot.'/img/icons/card.png" width="16" height="16" /></a>'
		.'<a target="_blank" href="'.CalRoot.'/link/index.php?tID=3&oID=0&lID=\'+location[0]+\'" title="'.$hc_lang_locations['MnuWeather'].'"><img src="'.CalRoot.'/img/icons/weather.png" width="16" height="16" /></a>'
		.'<a target="_blank" href="'.CalRoot.'/link/index.php?tID=2&oID=0&lID=\'+location[0]+\'" title="'.$hc_lang_locations['MnuDirection'].'"><img src="'.CalRoot.'/img/icons/car.png" width="16" height="16" /></a>'
		.(($hc_cfg[108] == 1) ? '<a href="webcal://'.substr(CalRoot, 7).'/link/SaveLocation.php?lID=\'+location[0]+\'" title="'.$hc_lang_locations['MnuiCalendar'].'"><img src="'.CalRoot.'/img/icons/ical.png" width="16" height="16" /></a>':'').''
		.(($hc_cfg[106] == 1) ? '<a target="_blank" href="'.CalRoot.'/rss/l.php?lID=\'+location[0]+\'" title="'.$hc_lang_locations['MnuRSS'].'"><img src="'.CalRoot.'/img/feed.png" width="16" height="16" /></a>':'');
	}
	/**
	...
	/**
	 * Deprecated, will be removed in 2.2. Use map_zoom() instead.
	 */
	function gmap_zoom($type = 0){
		echo map_zoom($type);
	}
	/**
	 * Deprecated, will be removed in 2.2. Use map_venue_lat() & map_venue_lon() instead.
	 */
	...
	/**
	 * Deprecated, will be removed in 2.2. Use icon URL instead.
	 */
	function gmap_pin_icon($img = ''){
		echo ($img != '') ? $img : CalRoot.'/img/pins/pushpin.png';
	}
	/**
	 * Deprecated, will be removed in 2.2. Use cIn() instead.
	 */
	function gmap_info($content){
		echo cIn($content);
	}
	/**
	 * Deprecated, will be removed in 2.2. Use map_infowindow_menu() instead.
	 */
	function gmap_infowindow_menu(){
		global $hc_cfg, $hc_lang_location;
		echo map_infowindow_menu();
	}
	/**
	 * Deprecated, will be removed in 2.2. Use map_locations() instead.
	 */
