 ...
 */
	...
	function rsvp(){
		...
		if(!hasRows($result) || $hc_cfg['IsRSVP'] == 0){
			echo '
	...
	function submit_update(){
		...
		$result = doQuery("SELECT e.*, l.PkID, l.Name, l.Address, l.Address2, l.City, l.State, l.Zip, l.Country
						...
						WHERE e.PkID = '" . $eID . "' AND e.IsActive = 1 AND e.OwnerID = '" . cIn($_SESSION['UserPkID']) . "'");
		...
		<p><a href="' . CalRoot . '/index.php?com=submit">' . $hc_lang_submit['ClickSubmitAgain'] . '</a></p>';
			...
			$imageURL = cOut(mysql_result($result,0,40));
			...
			$locCountry = ($locID == 0) ? cOut(mysql_result($result,0,37)) : cOut(mysql_result($result,0,49));
		...
		<fieldset>
			...
			<label for="eventDescription">'.$hc_lang_submit['Description'].'</label>
			<textarea name="eventDescription" id="eventDescription" rows="20" placeholder="'.$hc_lang_submit['PlaceDesc'].'">'.$eventDesc.'</textarea>
			<label for="cost">'.$hc_lang_submit['Cost'].'</label>
		...
		<fieldset>
			...
			<label for="eventRegistration">'.$hc_lang_submit['Registration'].'</label>
			<select name="eventRegistration" id="eventRegistration" onchange="togThis(this,document.getElementById(\'eventRegAvailable\'));">
				<option '.(($allowRegistration == 0) ? 'selected="selected" ' : '').'value="0">'.$hc_lang_submit['Reg0'].'</option>
				<option '.(($allowRegistration == 1) ? 'selected="selected" ' : '').'value="1">'.$hc_lang_submit['Reg1'].'</option>
			</select>
			<label for="eventRegAvailable">'.$hc_lang_submit['Limit'].'</label>
