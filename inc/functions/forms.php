 ...
 */
	...
	function rsvp(){
		...
		<fieldset>
			...
			<a href="'.CalRoot.'/index.php?eID='.$eID.'" class="output" rel="nofollow">'.cOut(mysql_result($result,0,0)).'</a>
			<label>'.$hc_lang_rsvp['Date'].'</label>
			<span class="output">'.stampToDate(mysql_result($result,0,1), $hc_cfg[14]).'</span>';
		...
		<input name="cancel" id="cancel" type="button" value="'.$hc_lang_rsvp['Cancel'].'" onclick="window.location.href=\''.CalRoot.'/index.php?com=detail&amp;eID='.$eID.'\';return false;" />
		</form>';
	}
	/**
	 * Output Event Submission JavaScript validation.
	 * @since 2.0.0
	 * @version 2.0.0
	 * @return void
	 */
	...
	function validate(){
		...
		if(document.getElementById("eventRegistration").value == 1){
			err +=reqField(document.getElementById("eventRegAvailable"),"'.$hc_lang_submit['Valid04'].'\n");
			err +=validNumber(document.getElementById("eventRegAvailable"),"'.$hc_lang_submit['Valid04'].'\n");
			err +=validGreater(document.getElementById("eventRegAvailable"),-1,"'.$hc_lang_submit['Valid03'].'\n");
	...
	function submit(){
		...
		<fieldset>
			...
			<textarea name="eventDescription" id="eventDescription" rows="20" placeholder="'.$hc_lang_submit['PlaceDesc'].'"></textarea>
		...
		<fieldset>
			<legend>'.$hc_lang_submit['RecurInfo'].'</legend>
			<label for="recurCheck">'.$hc_lang_submit['Recur'].'</label>
			<input name="recurCheck" id="recurCheck" type="checkbox" onclick="togRecur();" />
			<label class="blank">&nbsp;</label>
			<span class="frm_ctrls">
		...
		<fieldset>
			...
			<label for="eventRegistration">'.$hc_lang_submit['Registration'].'</label>
			<select name="eventRegistration" id="eventRegistration" onchange="togThis(this,document.getElementById(\'eventRegAvailable\'));">
				<option value="0">'.$hc_lang_submit['Reg0'].'</option>
				<option value="1">'.$hc_lang_submit['Reg1'].'</option>
			</select>
			<label for="eventRegAvailable">'.$hc_lang_submit['Limit'].'</label>
		...
		<fieldset>
			...
			getCategories('frmEventSubmit',$hc_cfg['CatCols'],NULL,$hc_cfg['CatLinks']);
		echo '
		</fieldset>';}
		echo '
		<fieldset>
	...
	function search_result(){
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			$keyword = (isset($_POST['keyword'])) ? cIn(strip_tags(str_replace("\"","'",$_POST['keyword']))) : '';
			$keyword .= (isset($_POST['hc_search_keyword'])) ? cIn(strip_tags(str_replace("\"","'",$_POST['hc_search_keyword']))) : '';
			...
			$doRecur = (isset($_POST['recurSet'])) ? cIn(strip_tags($_POST['recurSet'])) : 0;
		} else {
			...
			$keyword = (isset($_GET['k'])) ? cIn(strip_tags(str_replace("\"","'",htmlspecialchars($_GET['k'])))) : '';
			...
			$doRecur = (isset($_GET['r'])) ? cIn(strip_tags($_GET['r'])) : 0;
		}
		if($keyword != '')
			$sQuery .= " AND MATCH(Title,LocationName,Description) AGAINST('" . str_replace("'", "\"", cIn($keyword)) . "' IN BOOLEAN MODE) ";
		...
		if($doRecur == 1)
			$rQuery = " AND SeriesID IS NULL UNION
						...
						.$sQuery." AND e.IsActive = 1 AND e.IsApproved = 1 AND SeriesID IS NOT NULL GROUP BY SeriesID";
		...
		echo '<p>'.$hc_lang_search['ResultLabel']. ' [<a href="'.CalRoot.'/index.php?com=searchresult&amp;r='.$doRecur."&amp;s=".urlencode($startDate)."&amp;e=".urlencode($endDate).'&amp;k='.urlencode($keyword).$link.'">'.$hc_lang_search['ResultLink'].'</a>]';
	...
	function news_signup(){
		...
		<fieldset>
			<legend>'.$hc_lang_news['DeleteLabel'].'</legend>
			'.$hc_lang_news['DeleteNotice'].'
		</fieldset>
		...
		$uID = $occupation = 0;
		$firstname = $lastname = $email = $zipcode = $birthyear = $gender = $refer = $format = $yrOpts = '';
		$query = NULL;
	...
	function filter(){
		...
		$cnt = 0;		
		$cookie = (isset($_COOKIE[$hc_cfg[000].'_fc']) || isset($_COOKIE[$hc_cfg[000].'_fn'])) ? ' checked="checked"' : '';
		$cities = getCities();
	...
	function location_select($warn = true){
		...
		if($hc_cfg[70] == 1){
			...
			<input type="text" name="locSearchText" id="locSearchText" onwebkitspeechchange="searchLocations();" onkeyup="searchLocations();" size="30" maxlength="100" placeholder="'.$hc_lang_submit['PlaceLocSearch'].'" value="" x-webkit-speech />
