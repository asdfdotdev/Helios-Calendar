 ...
 */
	...
	function user_manage_account(){
		...
		$result = doQuery("SELECT NetworkType, NetworkName, NetworkID, Email, FirstSignIn, Level, Location, Birthdate
	...
	function user_manage_events(){
		...
		$result = doQuery("SELECT DISTINCT e.PkID, e.Title, e.StartDate, e.StartTime, e.EndTime, e.TBD, e.IsApproved, e.SeriesID
						...
						GROUP BY e.PkID, e.Title, e.StartDate, e.StartTime, e.EndTime, e.TBD, e.IsApproved, e.SeriesID
		...
		while($row = mysql_fetch_row($result)){
			...
			echo '
				<li class="row'.(($cnt % 2 == 0) ? ' hl' : '').'">
					...
					echo $time.'</div>
					<div class="ue_status txt">'.$hc_lang_user['Status'.$row[6]].'</div>
	...
	function user_register_new($network,$net_name,$net_id){
		...
		doQuery("INSERT INTO " . HC_TblPrefix . "users(NetworkType, NetworkName, NetworkID, FirstSignin, LastSignIn, LastIP, Level, IsPrivate)
				Values('".$network."','".cIn(utf8_decode($net_name))."','".cIn($net_id)."','".SYSDATE.' '.SYSTIME."','".SYSDATE.' '.SYSTIME."','".cIn(strip_tags($_SERVER["REMOTE_ADDR"]))."','0','1')");
