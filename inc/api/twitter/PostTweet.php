		...
		if($fp = fsockopen($ip, 80, $errno, $errstr, 1)){
			...
			if($showStatus > 0){
				...
				if($fetched[0]['name'] == 'error'){
					...
					for($i=0;$i < $stop;$i++){
						if(isset($fetched[0]['elements'][0]['elements'][$i]) && $fetched[0]['elements'][0]['elements'][$i]['name'] == 'id'){
							$tweetID = $fetched[0]['elements'][0]['elements'][$i]['value'];
						}else if(isset($fetched[0]['elements'][0]['elements'][$i]) && $fetched[0]['elements'][0]['elements'][$i]['name'] == 'screen_name'){
