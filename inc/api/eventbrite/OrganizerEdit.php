	...
	if(!hasRows($result)){
		...
		if(mysql_result($result,0,1) == '' || mysql_result($result,1,1) == ''){
			...
			if(!($fp = fsockopen($ip, 80, $errno, $errstr, 1)) ){
				...
				$ebSend .= ($eventbriteOrgN != '') ? '&name=' . urlencode(utf8_encode(htmlentities($eventbriteOrgN))) : '';
				$ebSend .= ($eventbriteOrgD != '' ) ? '&description=' . urlencode(utf8_encode(htmlentities($eventbriteOrgN))) : '';
